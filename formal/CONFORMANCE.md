...
| dvm-control-relay | `libs/driver-domain-host/src/lib.rs` challenge/proof/WELCOME and `new_input_epoch`; `driver-domains/linux/package/rustos-dvm-agent/src/rustos-dvm-agent.c` fw_cfg HMAC proof; `kernel/io-manager/src/input/dvm_serial.rs` RDI2 ingress | Fixed during this audit: CID and static HELLO no longer authorize the relay. L0 now requires a fresh per-launch HMAC-SHA256 proof over the exact HELLO before WELCOME, probe, or input authority; the agent reads the secret only from fw_cfg's root-only raw attribute. Relay epochs are atomically monotonic for a hostd lifetime and fail closed before reuse on wrap. |
...
| dvm-network-control | `libs/driver-domain-host/src/lib.rs` RDI1 session lifecycle; `kernel/io-manager/src/input/dvm_serial.rs`; `kernel/io-manager/src/io/dvm_network.rs` | Fixed during this audit: mapping a valid ivshmem aperture no longer reports a live network by itself. Only the active L0-authenticated one-shot control epoch permits transmit/receive; its exact end returns NoDevice, while stale cleanup and DVM-writable data-plane state cannot revoke or create a newer lease. |
| dvm-input-revocation | `libs/driver-domain-host/src/lib.rs` epoch allocation; `libs/driver-domain-protocol/src/lib.rs` shared `RustosInputFrame` plus Kani harnesses; `kernel/io-manager/src/input/dvm_serial.rs`, `input/event_queue.rs`; `services/inputd/src/main.rs`; `drivers/libs/keyboard-core/src/lib.rs` | Fixed during this audit: every session start/end now injects a priority reset barrier, inputd accepts that exact flag combination, and keyboard reset emits provider-owned releases before state is discarded. L0 never reuses a relay epoch within a hostd lifetime, so a retired sequence cannot restart after a later barrier. Kani proves the shared host wire constructor rejects invalid key/pointer input and preserves symbolic header/field provenance; it additionally checks independent fixed key/pointer CRC32 wire vectors. |
| dvm-input-write-deadline | `libs/driver-domain-host/src/lib.rs` `UnixInputSink::{queue_input_frame,flush_pending_nonblocking,finish_input_relay,wait_for_receiver_ready}`, `configure_input_socket_buffer`; `kernel/io-manager/src/input/dvm_serial.rs` `announce_receiver_ready` | Fixed during this audit: L0 waits for the private exact `RDRY` sequence before requesting the DVM input stream. It scans only a 4 KiB bounded prelude to tolerate firmware terminal-control bytes on COM2; anything else fails closed. RustOS emits `RDRY` only after the task-context COM2 broker drain is active. L0 uses a fixed 30-second-at-rate socket buffer (245,760 bytes) plus a fixed in-memory FIFO; normal traffic cannot consume the reserved session-end/release capacity. Nonblocking partial writes retain their byte offset and FIFO head, and `finish_input_relay` either drains the admitted queue in order or reports its bounded terminal failure. |
| dvm-input-drain-ownership | `kernel/io-manager/src/input/dvm_serial.rs` `service_pending`; `kernel/compat/src/user/syscall/linux/{input_broker_ops.rs,service_ops/poll_epoll.rs}` | Fixed during this audit: the RTC transport callback no longer runs decoder work. Only a capability-gated task-context input broker acquires `DECODER`, reads COM2, validates RDI2, and fills bounded ingress. Scheduler ticks have no decoder authority, eliminating the same-task recursive `KernelSpinLock<Decoder>` acquisition observed under input load. |
...
| input-readiness | `kernel/io-manager/src/input/event_queue.rs`; `kernel/compat/src/user/syscall/linux/{ipc_ops.rs,service_ops/poll_epoll.rs,service_ops/ipc_helpers.rs}`; `services/inputd/src/main.rs` `serve`/`dispatch`/`dispatch_read` | Fixed during this audit: inputd no longer periodically drains ring0 ingress ahead of poll readers or polls its endpoint with a sleep. It blocks in `SYS_RUSTOS_IPC_RECV`; each task-context poll recheck or authorized direct read first invokes the bounded COM2 broker drain, then transfers ingress through `INPUTD_IPC_OP_STATS` before observing service-owned readiness. The non-consuming probe has a 16 ms deadline, and TLC constrains every record to a DVM-labelled Linux key or pointer packet. Retry observes either ring0 ingress or the already-transferred policy record, preserving arm/recheck/wake without a stale empty policy queue. |
...
| ui-input-motion | `driver-domains/linux/package/rustos-dvm-agent/src/rustos-dvm-agent.c` `input_selftest_emit_cycle`; `services/uiserver/src/{input_loop.rs,main.rs}` | Fixed during this audit: the KVM-only DVM selftest now reverses relative x/y motion on short independent phases instead of driving the cursor permanently into an edge. The performance gate therefore samples sustained visual work rather than accepted-but-clamped input; production hardware input behavior is unchanged. |
...
| ipc-priority-inheritance | `kernel/ps/src/multitask/{scheduler.rs,current.rs}`; `kernel/compat/src/user/syscall/linux/ipc_ops.rs` | Fixed during this audit: every live synchronous reply can carry one bounded caller-to-receiver donation. Its effective System class crosses nested calls, while completion, timeout cancellation, and either task's retirement revoke the exact donation before a stale boost can survive. |
...
- The DVM input-write-deadline model treats Unix-socket write outcomes as
  adversarial but atomic partial-write outcomes. It proves the host relay's
  retry/terminal-result contract, not QEMU chardev scheduling or guest serial
  receiver progress.
