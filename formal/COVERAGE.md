...
| --- | --- | --- |
| A host NVMe/AHCI driver and VFIO overlap controller authority, any whole-device partition remains mounted/in swap/held, flush is skipped, an unsigned epoch or stale DVM readiness is admitted, the aperture is revoked before exact QEMU exit, or the original driver is restored before aperture revoke | boot-storage-handoff | tools/hostd storage admission/supervision, L0 epoch signer, and durable driver-domain-host VFIO lease |
