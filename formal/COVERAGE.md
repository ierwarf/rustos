...
| The storage-DVM QEMU gate reports failure or success because of an unrelated GPU/UI marker, omits its storage boot trace or first read completion, manually interprets an expected injected `block.flush` failure, accepts both injected failure and fabricated media-barrier liveness success, treats that read-only barrier as backing-image write durability, advertises FUA, or substitutes unsupported read-only `ide-hd`/invalid size properties for the existing-AHCI `ide-cd` 2048-byte FLUSH-only media contract | boot-storage-handoff; dvm-volume-io | xtask `--storage-dvm-only` independent positive profile plus exact-rule `--storage-dvm-expect-flush-fault` negative profile |
...
| An installed block aperture is signed before the copied private snapshot and containing directory are synced, accepts a missing/forged L0 signature, lacks exact built-in `CONFIG_BLK_DEV_SR=y` plus verifier enforcement and the S12 built-in-`sr`/modular-`sd_mod` closure, accepts non-2048-aligned ATAPI media or signed/ready geometry other than 2048/2048, leaves a false RustOS-ready bit after a peer race, treats initial DVM-not-ready as a terminal fault, spins instead of sleeping, loses readiness between check and scheduler arm, falls back to bootstrap storage, uses the volume before observing the exact generation, or declares restart recovery from a rebound marker without the rotate-returned successor generation and RustOS/DVM/READ_ONLY flags | dvm-block-startup | file-and-directory snapshot sync before signing, signed early-system key, read-only existing-AHCI ATAPI/driver contract, conditional readiness publication, kernel/io-manager readiness predicate, compat block-broker wait, services/storaged bounded startup loop, and xtask exact-successor recovery admission |
...
| The first independently prepared child runs before later siblings become runnable, a malformed/duplicate/foreign batch partially activates, a PID-equal process/MM-generation replacement is consumed after the original target snapshot, capability consumption precedes complete scheduler preflight, runnable publication precedes one-shot capability consumption, concurrent exit interleaves the lock-held consumption-to-publication interior, an impossible post-preflight failure is returned as recoverable state, or the synchronous loader reply delays a committed cohort's bounded first turns | atomic-process-activation-batch plus bootstrap-activation-handoff | initd fixed cohort construction, loaderd stamped-sender binding, kernel/compat pre-lock ProcessIdentity snapshots plus final-lock full-identity revalidation, ProcBrokerRegistry-to-Scheduler transaction, an executable post-authority suspension assertion, and kernel-ps allocation-free FIFO publication with a non-extendable cohort prefix |
...
| A fork child runs before the parent's writable translation is invalidated, a shared mapping lacks an exact root/VA/frame record, competing write faults consume one reference twice, `mprotect` bypasses COW, a file-private backing is promoted in place, rollback leaves a child alias, or unmap/retirement reuses the frame before the final acknowledgement | cow-frame-lifecycle | kernel/ps VMA fork hold, kernel/mm frame descriptor and present-write replacement, kernel/compat Linux fork transaction, and kernel/executive fault classification |
| Two owners load one backing page, a cache hit crosses service/object/backing generation, private COW dirties or aliases its shared-clean source, a dirty page reclaims without exact writeback, provider restart preserves stale authority or drops a mapped/dirty frame, dirty recovery changes object identity, or a frame releases before exact TLB acknowledgement | page-cache-lifecycle | services/pagerd/src/page_cache.rs and libs/rustos-user-abi/src/pager.rs |
| A pager observes a physical address, a stale generation or mismatched fault binding consumes a frame, one grant transfers twice, rights escalate, or a writable executable frame is published | pager-frame-grant-lifecycle | kernel/mm/src/memory/frame_capability.rs |
| A fault slot publishes a malformed request or endpoint, blocks without an exact token, accepts a reply before the pager block transition, accepts a stale or repeated reply, or reaches exception entry through allocation, `ProcessStateLock`, or a service lookup | pager-fault-slot-lifecycle | kernel/ps/src/multitask/pager_fault.rs |
| A partial unmap leaves ring0 holding remainders the pager has deleted, a surviving fragment keeps the wrong backing offset, an `mprotect` narrowing never reaches the pager so it grants rights the process no longer has, or a split that cannot fit drops a region instead of refusing | pager-region-agreement | libs/rustos-user-abi/src/pager/region_edit.rs, kernel/ps/src/multitask/pager_vma.rs and services/pagerd/src/lib.rs |
| A partially written VMA becomes visible, overlapping or W+X authority publishes, a stale process/MM/VMA generation or disallowed access resolves, exception lookup takes a process-state lock, or exact revoke leaves a match | pager-vma-publication | kernel/ps/src/multitask/pager_vma.rs and kernel/ps/src/multitask/current.rs |
...
| A futex task owns duplicate waiters, requeue followed by timeout leaves a stale old-key entry, forced/fault retirement leaves task-scoped futex, wait-set, endpoint, input, block, or exec-transition authority, robust owner death ignores the pending entry, walks an unbounded/cyclic user list, changes a foreign owner's word, loses the waiter bit, or a slot is reused before exact cleanup acknowledgement | futex-waiter-lifecycle | kernel/ps/src/multitask/{scheduler.rs,current.rs}, kernel/executive/src/boot.rs, kernel/compat/src/user/syscall/linux/{service_ops/futex_thread.rs,waitset_broker_ops.rs,ipc_ops.rs}, and kernel/io-manager/src/{input/wait_queue.rs,io/dvm_block.rs,sync.rs} |
| One process consumes all endpoint, shared-region, backing-byte, or scheduler-task capacity; admission allocates before reserving owner quota; a failed publication leaks its charge; or a dropped region returns quota before deferred physical backing is actually reclaimed | kernel-resource-accounting | kernel/ipc-runtime/src/{ipc/mod.rs,api.rs} and kernel/ps/src/multitask/process_table.rs |
| An x86 error-code exception enters ordinary Rust cleanup with a misaligned SysV call stack, diagnostics dereference an untrusted saved user RSP, NMI re-enters a lock held by the interrupted context, a recovered user fault loses authority, non-final thread retirement revokes a live process endpoint, final retirement retains task/process wait or IPC authority, or a kernel fault is misrouted through user cleanup | exception-retirement-lifecycle | kernel/hal/src/arch/{gdt.rs,idt/handlers.rs,idt/mod.rs}, kernel/executive/src/lib.rs, kernel/compat/src/user/syscall/mod.rs, and kernel/ps/src/multitask/scheduler.rs |
| A stale/malformed procd reply consumes a masked or non-pending signal, installs a non-user handler/restorer, permits SIGKILL ignore/handler or non-termination, maps a default stop to termination, stops only one thread, prevents SIGCONT/SIGKILL from resuming a stopped process, reports stopped/continued status without exact wait options, restores either unblockable mask, or fatal retirement retains process/task-IPC authority | process-signal-delivery | services/procd/src/main.rs, kernel/executive/src/lib.rs, kernel/compat/src/user/syscall/linux/{support.rs,syscalld_ops.rs,offload_ops.rs}, and kernel/ps/src/multitask/{current.rs,scheduler.rs,process_table.rs} |
| Parent SIGCHLD loses a stop/continue/exit cause, `SA_NOCLDSTOP` suppresses a coalesced exit, a stale procd snapshot clears a cause already consumed elsewhere, or a cause queued during selection is cleared by the older snapshot | sigchld-notification | libs/rustos-user-abi/src/{linux.rs,syscall.rs}, services/procd/src/main.rs, kernel/compat/src/user/syscall/linux/support.rs, and kernel/ps/src/multitask/scheduler.rs |
| Concurrent netd workers detach AF_UNIX poll batches and admit beyond the global bound, an expired request reserves a slot or starts an AF_INET provider under the state lock, detach/requeue refreshes the admitted absolute end, a rejected reply-bound AF_INET bind leaves its initial token live or discards it twice, an accepted request stays queued/detached forever, queue poisoning strands reservations/reply capabilities, a failed reply is confused with delivery, or a request attempts more than one terminal reply | netd-deferred-reply; ipc-reply-deadline | libs/rustos-user-abi/src/syscall.rs; kernel/{ipc-runtime,compat,ps}/src; services/netd/src/main.rs |
...
| Exec races thread creation, a target or sibling is detached while still running remotely, process exit/exec reselects a retired sibling whose exact cleanup still owns its slot, an old process generation is replaced before all live siblings retire, or page-table frames are returned while any CPU still owns the old root | cross-cpu-task-retirement | kernel/ps/src/multitask/{current,process_table,scheduler.rs}, kernel/ps/src/multitask/scheduler/smp.rs, kernel/hal/src/arch/tlb_shootdown.rs, and kernel/mm/src/memory/address_space.rs |
| Robust owner death loses a concurrent user update, clears a foreign owner, drops FUTEX_WAITERS, wakes before OWNER_DIED publication, retries without bound, or clear_child_tid becomes visible after its wake | thread-exit-futex-cleanup | `robust-futex-owner-death/RobustFutexOwnerDeath`; kernel/mm/src/memory/address_space/atomic_user.rs and kernel/compat/src/user/syscall/linux/service_ops/futex_thread.rs |
| An AP becomes Online without a private clockevent, uses an uncalibrated timer, shares the BSP PIT, rearms in the past, schedules before rearm, sends the wrong interrupt-controller EOI, or an empty single-user/Idle periodic edge takes the global scheduler while a real local, foreign, deferred, or return request is incorrectly ignored | per-cpu-clockevent-lifecycle | kernel/hal/src/arch/{clock,msi,timer}.rs, kernel/hal/src/arch/idt/mod.rs, and kernel/ps/src/multitask/{spawn,irq,cpu_local}.rs |
...
| A provider supplies foreign, unsupported, writable flags=0, composite flags=3, any nonexact `READ_ONLY` authority, zero, or byte-length-overflowing block geometry; a malformed FAT image mounts; a DVM-volume request is empty, unaligned, wraps its LBA, crosses admitted geometry, loses chunk accounting, accepts a foreign/stale/oversized storaged bulk reply, widens read authority for batching, collapses timeout/revocation into a generic device failure, or caches a transient metadata failure as a permanent negative | dvm-volume-io | libs/storage-fat/src/lib.rs, libs/rustos-user-abi/src/syscall.rs, services/vfsd/src/{lib.rs,block.rs}, services/storaged/src/block.rs, kernel/compat/src/user/syscall/linux/block_broker_ops.rs, and kernel/io-manager/src/io/dvm_block.rs |
| A storaged read cache exceeds its fixed memory or in-flight budget, amplifies a random miss into multi-window traffic, serves a range outside the cached window, retains overlapping aliases, crosses a DVM generation, publishes a partial failed batch, leaves issued tickets uncancelled, or survives a write/restart | dvm-read-cache | services/storaged/src/block.rs |
...
| A syscall compiler prologue clobbers XMM before the user snapshot, a blocking syscall aliases the entering user's SIMD/FPU image with the scheduler continuation slot, allows nested capture, or restores a snapshot to a different task | syscall-simd-lifecycle | kernel/ps/src/multitask/{scheduler.rs,spawn.rs} and kernel/compat/src/user/syscall/mod.rs |
| A syscall wake validates a consumed running frame, block fails to publish the continuation atomically with CPU transfer, blocked wake loses the published frame, dispatch fails to consume it, the syscall entry frame is released while blocked, or SYSRET trusts validation performed before the last resume | syscall-scheduler-continuation | kernel/ps/src/multitask/{scheduler.rs,irq.rs} and kernel/compat/src/user/syscall/mod.rs |
...
| Continuously runnable System work consumes every dispatch while User work remains runnable; one CPU consumes another CPU's fairness streak, virtual timeline, or handoff/activation authority; an idle CPU never pulls eligible work from a loaded peer, server boost searches only the caller CPU, stealing takes two runqueue locks, or a repeated rehome fills a finite mailbox with stale generations of one slot; an unadmitted User wall-clock deadline overrides weight-normalized vruntime under overload; a last-CPU preference crosses class/affinity/ownership or exceeds its finite same-class vruntime lag; initd, runtimed, or netd wakes an otherwise idle Ring3 poller every 1–2 ms; a latency handoff FIFO overwrites an owner, admits duplicates/System tasks, retains stale owners, grows without bound, or consumes an unbounded dispatch burst; runtime, scheduler-entry, real-switch, address-space-switch, migration, or catalog-lock attribution is unbounded, non-destructive, incomplete, aliases a reused task slot, influences scheduling policy, or is rendered under the scheduler owner; or a preemptible boot-random master owner leaves a higher-class kernel caller spinning forever | scheduler-cpu-distribution | kernel/ps/src/multitask/scheduler.rs, kernel/ps/src/multitask/scheduler/{locality.rs,runtime_profile.rs,runqueue.rs,runqueue_policy.rs}, kernel/ps/src/multitask/cpu_local.rs, kernel/ps/src/multitask/irq.rs, services/{initd,runtimed,netd}/src/main.rs, kernel/compat/src/user/syscall/linux/ipc_ops.rs, and libs/boot-random/src/lib.rs |
| A source active-balance cadence depends on RTC residue, skips its first or each eighth loaded opportunity, transfers an execution- or transition-owned continuation, duplicates custody, omits the exact target mailbox/reschedule request, drains/dispatches without that custody, or redirects work outside selected target affinity | scheduler-active-balance | kernel/ps/src/multitask/scheduler/runqueue_policy.rs |
| A uiserver helper retains inherited System class or elevated permanent fair weight, self-demotion raises an already lower weight or erases a live reply-scoped priority donation, or loaderd/vfsd drop their boot-critical class before the exact terminal UI reply completes | scheduler-thread-demotion | kernel/ps/src/multitask/scheduler.rs, kernel/compat/src/user/syscall/linux/scheduler_ops.rs, services/uiserver/src/{sys.rs,main.rs,input_loop.rs,runtime_sync.rs}, services/loaderd/src/main.rs, and services/vfsd/src/main.rs |
| A fast byte call exceeds 256 bytes, targets a non-front or stale receiver, loses exact caller process/task or MM identity, publishes both direct and queued custody, redirects a cross-CPU transfer, partially falls through after commit rejection, leaks reply scheduling custody, or lets the compact syscalld wrapper accept a foreign sender or unsupported operation | ipc-fast-handoff | `formal/ipc-fast-handoff/IpcFastHandoff.tla`; kernel/ipc-runtime/src/{api.rs,ipc/mod.rs}; kernel/ps/src/multitask/{scheduler.rs,scheduler/{runqueue,smp}.rs}; kernel/compat/src/user/syscall/linux/ipc_ops.rs; services/syscalld/src/fast_offload.rs |
| Concurrent call/reply transfers overwrite a required peer; a terminal reply releases donation, wakes, and captures its token in separate Scheduler transactions; a post-drop token publishes on a noncaptured CPU or after task-ID/generation/runnability/Local-or-RemoteQueued custody changed; selection dispatches a still-RemoteQueued record before its target mailbox drains to Local; a record is redirected into another CPU's FIFO; CPU-local deduplication is accidentally global and suppresses a fresh post-migration record; one CPU is charged for another CPU's handoff burst or pops its FIFO head before the forced ordinary turn; Generic enqueue downgrades Reply custody; a migrated-away-and-back stale generation regains urgency at selection; a stale token takes a catalog/global fallback or publishes twice; two tasks share one publication-pending bit; two independently RemoteQueued replies are hidden by a task-role symmetry assumption; owner churn or retirement of one reply corrupts the other; or IPC chains suppress fairness without bound | synchronous-ipc-handoff | `formal/synchronous-ipc-handoff-concurrency/SynchronousIpcHandoffConcurrency.tla`; kernel/compat/src/user/syscall/linux/{ipc_ops.rs,ipc_reply_recv.rs}; kernel/ps/src/multitask/{current.rs,scheduler.rs,scheduler/{sync_handoff,runqueue,smp,handoffs}.rs} |
| Fused reply-receive collides with another native syscall number, consumes a one-shot cap before validating user shape, receives before reply commit, loses the endpoint waiter wake, aliases pre/post-commit errors, accepts a reserved result as retryable, fails to recover a proven-live pre-commit cap, retries a completed cap, strands a malformed dequeued caller without a terminal reply, or lets loaderd block before required descriptor cleanup or scheduling demotion | ipc-reply-recv-transaction | libs/rustos-user-abi/src/syscall/ipc_reply_recv.rs, libs/rustos-svc-runtime/src/ipc.rs, kernel/compat/src/user/syscall/linux/{ipc_ops.rs,ipc_reply_recv.rs}, services/inputd/src/service_loop.rs, and services/loaderd/src/{lib.rs,main.rs} |
| Stale service endpoint or capability after revoke/exit | endpoint-registry | kernel/compat/src/user/syscall/linux/ipc_ops.rs |
| A receiver publishes a waiter after observing that a message is already pending, allowing a later producer to consume stale wake or synchronous-handoff authority for a task that never blocked | endpoint-receiver-wakeup | kernel/ipc-runtime/src/ipc/mod.rs, kernel/compat/src/user/syscall/linux/ipc_ops.rs, and kernel/ps/src/multitask/scheduler.rs |
...
| Independent post-init activation is serialized behind unrelated readiness, a spawned child satisfies dependencies before exact endpoint admission, runtimed/storaged starts with a partial barrier, or a foreign/stale PID publication is accepted | post-init-bootstrap-barrier | services/initd/src/main.rs and rootd exact post-init leases |
| A later child activation or loader reply overwrites an earlier committed child's first-turn authority, duplicate activation consumes queue capacity, stale retirement erases another child, an overdue fairness turn consumes the handoff, or a live child is dispatched out of activation order | bootstrap-activation-handoff | kernel/ps/src/multitask/scheduler.rs and kernel/ps/src/multitask/spawn.rs |
| A same-CID process lacks the per-launch challenge proof yet gains control authority; a foreign DVM, mismatched reply, stale input epoch, or out-of-order relay frame gains authority | dvm-control-relay | libs/driver-domain-host/src/lib.rs, driver-domains/linux/package/rustos-dvm-agent/src/rustos-dvm-agent.c, services/inputd/src/dvm_protocol.rs |
...
| A mapped DVM Ethernet aperture remains usable after its authenticated control session ends, a stale end tears down a newer session, a foreign service grants a lease, or DVM-writable data-plane state creates network authority | dvm-network-control | libs/driver-domain-host/src/lib.rs, services/inputd/src/dvm_protocol.rs, services/netd/src/main.rs, kernel/io-manager/src/io/dvm_network.rs |
...
| A DVM gains a write path to the host-owned ring, two process-authorized callers concurrently drain the sole consumer cursor, L0 produces before a live policy consumer, cursors exceed the fixed aperture, IRQ decodes or moves cursors, revoke omits a service-visible barrier, a stale/malformed record reaches policy, recovery reallocates a permanent MSI-X vector or leaks a mapping, or finite committed work never drains | dvm-input-ring | libs/driver-domain-protocol/src/lib.rs, libs/driver-domain-host/src/{lib.rs,ivshmem.rs}, kernel/io-manager/src/input/{dvm_ring.rs,wait_queue.rs}, services/inputd/src/dvm_protocol.rs, kernel/compat/src/user/syscall/linux/input_broker_ops.rs |
| A storage DVM chooses guest/host addresses, overruns a queue or transfer slot, completes a foreign/stale request, fabricates stable-write evidence or loses FLUSH/FUA ordering on a writable transport, makes the read-only qualification media advertise FUA or treats its media-barrier completion as write durability, preserves queue authority across restart, or forges a successor generation | dvm-block-transport | protocol ABI v2, kernel signed-epoch admission/rebind, Linux relay, storaged/vfsd, and KVM shared-ring evidence |
...
| Vfsd admits an unbounded number of persistent epoll objects or interests, a retired epoll retains membership, or a duplicate/over-capacity checkpoint batch partially replaces the live registry | userspace-wait-set/WaitSetRegistry | libs/rustos-user-abi/src/performance.rs and services/vfsd/src/{lib.rs,state_checkpoint.rs} |
...
| A readiness edge lands between the UI loop's check and sleep, coalesced notifications erase the only wake, scheduler block commit is separated from reschedule, or a bounded compositor deadline never resumes the waiting thread | ui-main-loop-wakeup | services/uiserver/src/{input_loop.rs,main.rs,wayland.rs}, kernel/hal/src/{hooks.rs,arch/rtc.rs}, and kernel/ps/src/multitask/irq.rs |
| Input and Wayland damage are split across redundant early presentations; a Wayland frame callback runs without a previous real presentation or damage-free cadence permit; missed timer pulses accumulate callback credit; pointer motion grows an unbounded client queue or is reordered after a button transition; or pending damage/callback work can remain live forever under the declared scheduler/timer fairness assumptions | wayland-frame-pacing | services/uiserver/src/{main.rs,wayland.rs} |
| A netd-backed accept starts without listener readiness, fixed-cadence probing consumes idle service turns, a blocking accept owns the uiserver frame loop, accepted clients accumulate without a bound, overload leaks a client stream, or a queued accepted client never settles under the declared accept/UI fairness | wayland-accept-isolation | services/uiserver/src/wayland_accept.rs, services/uiserver/src/wayland.rs, services/uiserver/src/main.rs, and services/netd/src/main.rs |
...
| A partial X/Y report publishes an absolute pointer position, a duplicate complete report produces phantom motion, an out-of-bounds coordinate reaches UI state, ownership is duplicated between pipeline stages, or finite accepted absolute-position work never reaches presentation | dvm-absolute-pointer | driver-domains/linux/package/rustos-dvm-agent/src/rustos-dvm-agent.c, libs/driver-domain-{protocol,host}/src, services/inputd/src/{dvm_protocol.rs,main.rs}, and services/uiserver/src/app/input.rs |
...
| GUI-DVM scheduling races RustOS for ivshmem peer 0, a DVM connects without the pinned RustOS peer, a replacement receives a new logical identity/eventfd lease, a cohort member cannot address a retained disconnected slot, or reconnect is mistaken for a new authenticated display/block epoch | ivshmem-pairing | libs/driver-domain-host/src/ivshmem.rs and tools/xtask/src/kvm/{guest,layout}.rs |
...
| A private UI frame publishes commands without its immutable atlas generation, maps sibling slots or command pages into the compositor, carries a user pointer in the ring0 commit, loses write-combine PAT state during `mprotect`, admits an unregistered or backend/mode-mismatched GPU profile, uses an old or ambiguous prime source-mode value, submits a mode different from the authenticated prime, initializes a new DVM texture from partial/no damage, applies partial damage to a backing slot without an exact predecessor or complete bounded contiguous history, coalesces disjoint history into an atlas-sized bounding rectangle, treats console content generation as scene topology and rebuilds the complete atlas, selects a stale released slot whose replay exceeds the bounded reconstruction budget instead of applying backpressure, overlaps damage records, executes texture updates out of submission order, reuses an atlas while the DVM still has read authority, executes a QEMU frame without its staged upload, reports staged copy as zero copy, presents before the GPU fence, reuses the old front before the KMS present fence, or retains source authority across revoke/reset | dvm-gpu-atlas-transport | libs/driver-domain-protocol/src/lib.rs, services/uiserver/src/{gpu_scene.rs,gpu_runtime.rs}, kernel/{io-manager,mm,ps} display-surface paths, and driver-domains/linux/package/rustos-dvm-display/src |
...
| A deadline-bounded IPC caller remains blocked after a reply, endpoint owner exit, or timeout; a late reply revives a cancelled call; a service-created resource becomes visible before the caller's post-take deadline decision or survives timeout/cancellation/endpoint exit; each rejected reply emits synchronous ring0 and ring3 diagnostics until debug output becomes a second workload; a readiness or policy-only call silently inherits the 30 s bulk-data ceiling; a granted service lookup repeats rootd authorization; every stable service call contends on the global endpoint mutation lock; or rootd/runtimed process one already-queued control request before re-entering unrelated slow policy | ipc-reply-deadline plus source and implementation-mutation witnesses | libs/rustos-user-abi/src/performance.rs, libs/rustos-svc-runtime/src/ipc.rs, kernel/ipc-runtime/src/ipc/{mod.rs,endpoint_priority.rs}, kernel/compat/src/user/syscall/linux/{ipc_ops.rs,ipc_reply_diagnostics.rs,net_broker_ops.rs}, services/{netd,inputd,devmgrd}/src/main.rs, services/rootd/src/{main.rs,control_drain.rs}, and services/runtimed/src/main.rs |
...
| A launcher presents multiple configured vCPUs as SMP evidence while any per-CPU scheduler/syscall/clockevent, CPU-online, reschedule-IPI, TLB-shootdown, robust-futex, topology ABI, or fault-injection prerequisite is absent; accepts a stale/source-mismatched verification seal; or omits online, idle, user, timer, or IPI evidence for any requested CPU | smp-release-admission | tools/xtask formal evidence validation, KVM launch admission, and exact per-CPU runtime marker matrix |
| A KVM-only Ring3 workload admits any worker count other than 1/2/4/8 or a DVM-substituted executable; private storage delays/rolls back the signed ordinary catalog, endpoint 0/`-ENOENT` is mistaken for mounted-file absence, or a private-loader/candidate error is swallowed rather than pending retry; a catalog entry forges injector-owned pre-UI provenance; qualification omits its DVM block provider, attaches a writable/mismatched disk, signs a writable header, rotates a writable predecessor, accepts dual-ready without `READ_ONLY`, or a Linux relay accepts `BLKROGET`/header disagreement; it may also omit the signed early-system `smpqual.elf`; runtimed activates before bind or leaves a failed child runnable; an unbound qualified child enters individual or batch activation; a stale SESSIOND epoch, owner/target process generation, or MM generation reaches a binding; activation refreshes/arms its deadline outside the one-shot transaction; a phase races an endpoint replacement after admission; Ready/Start/Finish/Complete breaks its exact worker/TID/CPU FSM or encoded binding/work word; evidence omits/replaces the actual DVM-disk or early-system-executable path/bytes/SHA-256 or skips prelaunch recheck; prelaunch hashing/copy or the bounded input relay consumes the post-spawn guest readiness budget; owner/target exit leaves authority for deferred/PID reuse; Linux debug or system-console Ring3 payload forges/quote-breaks a milestone line; or host treats a transport checksum as process authentication | smp-ring3-qualification | apps/smpqual/smpqual.c; libs/rustos-user-abi/src/syscall.rs; kernel/compat/src/user/syscall/linux/{debug_ops.rs,smp_qualification_ops.rs,proc_broker_ops.rs,proc_broker_ops/activation_batch.rs,../../sysops/console.rs}; kernel/nucleus-core/src/debug/mod.rs; services/runtimed/src/{catalog.rs,main.rs,socket.rs,spawn.rs,kvm_smp_qualification.rs}; services/vfsd/src/{lib.rs,block.rs}; tools/xtask/src/{stage/mod.rs,kvm.rs,kvm/evidence.rs,kvm/evidence/smp_qualification.rs,kvm/guest.rs,kvm/layout.rs,options.rs}; driver-domains/linux/package/rustos-dvm-block/src/rustos-dvm-block.c |
| Linux reports CPU zero after SMP admission; syscalld accepts a forged/stale/empty/count-mismatched topology stamp; Windows exposes an admitted mask through Microsoft-reserved `SYSTEM_BASIC_INFORMATION` bytes instead of deriving it from the dense count; a partial Online set becomes application ABI; or raw APIC identity escapes as a logical CPU number | cpu-affinity-observation | kernel-hal lifecycle snapshot, kernel-compat request stamp, shared Linux/Windows topology-observation ABI, syscalld pure admission, and winsys standard topology exports |
| Linux affinity targets a foreign process, returns the whole topology instead of the effective thread mask, or commits before policy admission; Windows process affinity partially updates live threads, thread affinity escapes the process mask, a failure NTSTATUS becomes a nonzero success mask, or current-processor observation escapes Online | task-affinity-lifecycle | kernel-ps scheduler affinity state, kernel-compat Linux/Windows adapters, syscalld affinity policy, shared pseudo-handle ABI, and winsys ntdll/kernelbase exports |
| Monotonic time is inferred from lossy RTC interrupt count, a delayed virtual clockevent extends every deadline, invariant-TSC rate is mistaken for cross-CPU offset admission, raw TSC becomes the SMP global source before skew normalization, or sleep reacquires the process-table lock already held by its syscall | clocksource-deadline | kernel/hal/src/arch/{acpi.rs,clock.rs,rtc.rs}, kernel/hal/src/hooks.rs, kernel/ps/src/multitask/{current.rs,scheduler.rs,irq.rs} |
...
| A runnable user task is published without an admitted rootd policy; direct bootstrap bypasses its one-shot exact grant; deadline utilization exceeds the per-CPU ceiling; a task consumes beyond its context/domain budget; refill loses or creates credit; a passive server bills the wrong context or reply; timeout identity/accounting is absent or retried without bound; or terminal settlement leaks borrowed authority | scheduling-context-budget | services/rootd/src/{main.rs,scheduling_policy.rs}, kernel/executive/src/boot.rs, kernel/ipc-runtime/src/{api.rs,ipc/mod.rs}, kernel/ps/src/{api.rs,multitask/{current.rs,spawn.rs,scheduling_api.rs,scheduler.rs},multitask/scheduler/{scheduling_context.rs,ipc_donation.rs,donation_ledger.rs}}, and kernel/compat/src/user/{console_host.rs,process/mod.rs,syscall/linux/{ipc_call_admission.rs,ipc_ops.rs,scheduler_ops.rs,proc_broker_ops/scheduling_context_grants.rs,service_ops/ipc_helpers.rs}} |
...
| Ring3 bytes are reinterpreted as a Rust enum/padded descriptor, a zero/forged/stale ID or nonce constructs or consumes authority, opaque IPC descriptors remain after queue cancellation, peer-close, invalid receiver output, or scheduler retirement, a transferred service-backed open description loses or leaks its reference, retired owners retain endpoint authority, or one batch is partially installed | ipc-handle-transfer | libs/rustos-user-abi/src/syscall.rs, kernel/ps/src/user/handles.rs, kernel/ipc-runtime/src/ipc/mod.rs, kernel/compat/src/user/syscall/linux/{ipc_ops.rs,service_ops/vfs_socket.rs,service_ops/vfs_meta.rs}, kernel/ps/src/multitask/scheduler.rs, and kernel/executive/src/boot.rs |
...
| A process bypasses rootd lookup policy by guessing a monotonic numeric endpoint, a stale lookup remains callable after service restart/republication, process exit leaves a call grant, or a foreign process invokes an unpublished generic endpoint | service-call-authority | kernel/compat/src/user/syscall/linux/ipc_ops.rs, kernel/compat/src/user/syscall/linux/ipc_ops/subject.rs, and kernel/ipc-runtime/src/ipc/mod.rs |
| Any local process forges UI readiness, launches a program, terminates a session/PID, or reads runtime state by writing a valid runtimed socket frame without owning the live uiserver service or a signed logical-admin launch; or a partial lower-class client makes the supervisor busy-wait and prevents signed launch policy from converging | runtime-control-authority | services/runtimed/src/{socket.rs,spawn.rs,main.rs}, netd SO_PEERCRED |
| A foreign process receives a process-owned endpoint, completes a guessed reply capability, binds a prepared descriptor to another receiver, installs a reply-bound AF_INET fd before the same deadline or without the exact token/domain/type/protocol/fd/status flags, prevents worker-thread service, leaves authority after owner-process exit, revives a dead numeric endpoint by enqueueing after its owner exits, any ordinary, transfer, `dup2`, or `F_DUPFD` install expands the ring-0 descriptor table beyond its declared ceiling or partially installs a capacity-rejected transfer batch, a malformed/locally rejected vfsd open retains remote authority, or a failed socket/socketpair/accept publication leaks new netd tokens or a partial fd pair | ipc-endpoint-ownership; ipc-reply-deadline | kernel/ipc-runtime/src/ipc/mod.rs, kernel/compat/src/user/syscall/linux/{ipc_ops.rs,net_broker_ops.rs}, kernel/ps/src/multitask/current.rs, kernel/ps/src/user/handles/table.rs, kernel/compat/src/user/syscall/linux/service_ops/{ipc_helpers.rs,vfs_socket.rs}, and services/netd/src/main.rs |
...
### DVM revoke and qualification-output coverage

The DVM block source witnesses cover the closed twelve-reason production
vocabulary, one pre-clear reason/generation/flags/cursor observation, Required
`dvm-block-transport-revoked` argument order, and the valid FLUSH-to-first-64
KiB-READ path. SMP qualification covers only its four explicit critical names,
qualification-local loss snapshots, global-plus-local critical-drop accounting,
one-attempt scheduler measurements, and host rejection of nonzero local loss.
...
The restart topology now has a current signed-image parallel KVM witness in
addition to its source/model evidence. The runtime terminal was reached within
the five-second product bound without recreating the rootd/loaderd/procd cycle,
panic, or stalled lifecycle drain. The same run passed the three-window 55 FPS WayClick,
uiserver-input, and DVM page-flip gates. Physical-GPU admission remains a
separate hardware-specific evidence boundary.
...
Vfsd commits live epoll identities and complete interest records to rootd's
authenticated, versioned opaque checkpoint before local publication. Ring0 is
the sole descriptor-reference authority, so dup/fork/transfer do not issue
provider mutations; vfsd sees only create and the idempotent final-retire
tombstone. A
...
Immutable desktop and launch-policy files use bounded explicit-offset snapshot
reads. They therefore retain their open-description cursor and cannot add a
prepare/commit checkpoint transaction to vfsd's single request executor while
latency-sensitive readiness and Wayland control traffic is pending. The source
witness seeds a nonzero cursor and the implementation-mutation gate replaces
the positioned read with a sequential read; that mutant must fail.
The private KVM acceptance contract uses the same primitive with a separate
256-byte protocol ceiling in both runtimed's launch injection and uiserver's
bounded late watcher. Source conformance rejects either consumer if it falls
back to a sequential cursor-mutating read.
...
health context. uiserver now grants one non-accumulating 15 ms cadence permit,
...
bounded 66.7 Hz/90-second producer, and steady-state uiserver submission never
...
ms inter-event gaps. The earlier dedicated uiserver reader used a
non-accumulating 4 ms direct-read cadence to bypass the
ring0/service-queue lost-wake race. That compatibility route is now retired:
the reader blocks on the generic provider-generation epoll contract, treats
revoke as ERR/HUP, and issues authorized READ only after positive readiness.
The agent now grants only an authenticated live input stream SCHED_RR
...
Coalesced presentation, exact damage, direct mapped atlas slots, bounded
readiness handoff, and ordinary User-class fair share subsequently removed the
steady bottlenecks without a client-specific route. The compositor now consumes
one permit from the previous real presentation before the next present and
phase-locks damage-free callback pulses with the 15 ms DVM presentation clock.
A pending callback also participates in the main wait deadline.

The current schema-9 bounded capture passed the exact 55-FPS command. The first
accepted three consecutive WayClick windows contained 57, 65, and 68 exactly
balanced commit/callback/release cycles over 3.060 seconds, for 62.092 FPS
aggregate, 56.394--67.342 FPS per window, and a 45 ms maximum callback gap.
The same capture passed the independently aligned uiserver FPS/input/cursor and
DVM relay/fence-count gates with no provider revoke, context loss,
compositor-offline, or `display not available` marker. The gate does not
combine disjoint compositor, client, or relay windows. Older schema-8
33--45 FPS measurements remain historical diagnostic evidence only.

The current schema-9 GUI/input F5 rerun verified display and storage in
4.214 seconds and then remained healthy for more than 60 seconds, passing the
independent five-second gate with the authenticated staged-copy compositor active on a 2048x2048
...
instead of rescanning PCI and logging the same absence every 50 ms. The
separate steady-state WayClick proof now passes as described above; the
five-second boot gate remains independent and cannot substitute for it.
...
route. The current parallel KVM witness exercises this live wait/event
workload: three consecutive WayClick callback and uiserver input/present
windows exceed 55 FPS with no loss/backlog, and the DVM page-flip relay
independently sustains the same bound.
...
direct-map cache mutation outside its three owners, and display-pixel raw PTE
updates that bypass the io-manager WC mapper.
