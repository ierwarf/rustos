...
| A task is current on two CPUs, a remote wake loses its local request or global fan-out custody, post-unlock fan-out omits an Online CPU, the flusher sends an unsupported fixed self-IPI instead of retaining its local bit, repeated wakeups storm IPIs, a raw-lock interruption consumes work, or a private fixed IPI targets an invalid CPU/vector | smp-reschedule-ipi-lifecycle | kernel/ps/src/multitask/{cpu_local,irq}.rs, kernel/ps/src/multitask/scheduler/smp.rs, kernel/hal/src/arch/msi.rs, and kernel/lowlevel/src/interrupts.rs |
...
| An AP becomes Online without a private clockevent, uses an uncalibrated timer, shares the BSP PIT, rearms in the past, schedules before rearm, or sends the wrong interrupt-controller EOI | per-cpu-clockevent-lifecycle | kernel/hal/src/arch/{clock,msi,timer}.rs, kernel/hal/src/arch/idt/mod.rs, and kernel/ps/src/multitask/{spawn,irq}.rs |
...
| Continuously runnable System work consumes every dispatch while User work remains runnable; an unadmitted User wall-clock deadline overrides weight-normalized vruntime under overload; a last-CPU preference crosses class/affinity/ownership or exceeds its finite same-class vruntime lag; initd, runtimed, or netd wakes an otherwise idle Ring3 poller every 1–2 ms; a latency handoff FIFO overwrites an owner, admits duplicates/System tasks, retains stale owners, grows without bound, or consumes an unbounded dispatch burst; runtime, scheduler-entry, real-switch, address-space-switch, migration, or global-lock attribution is unbounded, non-destructive, incomplete, aliases a reused task slot, influences scheduling policy, or is rendered under the scheduler owner; or a preemptible boot-random master owner leaves a higher-class kernel caller spinning forever | scheduler-cpu-distribution | kernel/ps/src/multitask/scheduler.rs, kernel/ps/src/multitask/scheduler/{locality.rs,runtime_profile.rs}, kernel/ps/src/multitask/cpu_local.rs, kernel/ps/src/multitask/irq.rs, services/{initd,runtimed,netd}/src/main.rs, kernel/compat/src/user/syscall/linux/ipc_ops.rs, and libs/boot-random/src/lib.rs |
