...
| Continuously runnable System work consumes every dispatch while User work remains runnable; one busy User hides another past its ready-age bound; a latency handoff FIFO overwrites an owner, admits duplicates/System tasks, retains stale owners, grows without bound, or consumes an unbounded dispatch burst; or a preemptible boot-random master owner leaves a higher-class kernel caller spinning forever | scheduler-cpu-distribution | kernel/ps/src/multitask/scheduler.rs, kernel/compat/src/user/syscall/linux/ipc_ops.rs, and libs/boot-random/src/lib.rs |
| A uiserver helper inherits permanent System scheduling authority, self-demotion erases a live reply-scoped priority donation, or loaderd/vfsd drop their boot-critical class before the exact terminal UI reply completes | scheduler-thread-demotion | kernel/ps/src/multitask/scheduler.rs, kernel/compat/src/user/syscall/linux/scheduler_ops.rs, services/uiserver/src/{sys.rs,main.rs,input_loop.rs,runtime_sync.rs}, services/loaderd/src/main.rs, and services/vfsd/src/main.rs |
