...
| inputd drains ring0 ingress before a poll-woken UI/input client can read it, leaving an event in a service-only queue while the kernel reports no readiness | input-readiness | kernel/io-manager/src/input/event_queue.rs, kernel/compat/src/user/syscall/linux/service_ops/poll_epoll.rs, services/inputd/src/main.rs |
