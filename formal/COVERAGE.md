...
| The storage-DVM QEMU gate reports failure or success because of an unrelated GPU/UI marker instead of block peer readiness, signed geometry, first completion, and the exact E2E flush | boot-storage-handoff | xtask `--storage-dvm-only` independent acceptance profile |
