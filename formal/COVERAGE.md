...
| The storage-DVM QEMU gate reports failure or success because of an unrelated GPU/UI marker, omits first completion, manually interprets an expected flush failure, or accepts both injected failure and a fabricated E2E success | boot-storage-handoff; dvm-volume-io | xtask `--storage-dvm-only` independent positive profile plus exact-rule `--storage-dvm-expect-flush-fault` negative profile |
...
| A pending raw acquisition consumes another guard's preemption pin, pending-to-held publication or failed-acquire cancellation is interruptible, a raw guard releases on another CPU/APIC identity, preemption depth underflows, an IRQ dispatches while any nesting remains, scheduler scratch is mistaken for remote ownership, current-task publication duplicates one running task, a ready scan reads a frame owned by any CPU, a real slot switch skips task-specific architecture restoration, a same-task turn redundantly flushes that state, or the outgoing task stack becomes remotely runnable before assembly switches `rsp` to the incoming frame | scheduler-cpu-ownership | kernel/nucleus-core/src/util/lockdep.rs, kernel/lowlevel/src/interrupts.rs, kernel/hal/src/interrupt_stubs.rs, and kernel/ps/src/multitask/{cpu_local,irq,scheduler}.rs |
| A CPU activation is excluded from a mutation snapshot without conservative targeting, retirement starts while a task/slot/process reference remains, a Retiring/Reclaimed root is activated again, an identical active root unnecessarily reloads CR3, a target is omitted or mismatched, stale translations survive acknowledgement, an acknowledgement aliases another generation, or a frame is reclaimed before every target flush | tlb-shootdown-lifecycle | kernel/hal/src/arch/tlb_shootdown.rs, kernel/mm/src/memory/{address_space,kernel_vm}.rs, kernel/mm/src/memory/address_space/rollback.rs, and kernel/ps/src/multitask/{scheduler,process_table}.rs |
...
| A provider supplies foreign, unsupported, unknown-flag, zero, or byte-length-overflowing block geometry; a malformed FAT image mounts; a DVM-volume request is empty, unaligned, wraps its LBA, crosses admitted geometry, loses chunk accounting, accepts a foreign/stale/oversized storaged bulk reply, widens read authority for batching, collapses timeout/revocation into a generic device failure, or caches a transient metadata failure as a permanent negative | dvm-volume-io | libs/storage-fat/src/lib.rs, libs/rustos-user-abi/src/syscall.rs, services/{vfsd,storaged}/src/block.rs, kernel/compat/src/user/syscall/linux/block_broker_ops.rs, and kernel/io-manager/src/io/dvm_block.rs |
...
| Concurrent call/reply transfers overwrite a required peer, a stale peer consumes another transaction's handoff, unrelated overdue work indefinitely delays a committed synchronous transaction, or IPC chains suppress global fairness without bound | synchronous-ipc-handoff | kernel/compat/src/user/syscall/linux/ipc_ops.rs and kernel/ps/src/multitask/{current.rs,scheduler.rs,scheduler/handoff_queue.rs,scheduler/handoffs.rs} |
...
| A deadline-bounded IPC caller remains blocked after a reply, endpoint owner exit, or timeout; a late reply revives a cancelled call; each rejected reply emits synchronous ring0 and ring3 diagnostics until debug output becomes a second workload; a readiness or policy-only call silently inherits the 30 s bulk-data ceiling; a granted service lookup repeats rootd authorization; every stable service call contends on the global endpoint mutation lock; or rootd/runtimed process one already-queued control request before re-entering unrelated slow policy | ipc-reply-deadline plus source and implementation-mutation witnesses | libs/rustos-user-abi/src/performance.rs, libs/rustos-svc-runtime/src/ipc.rs, kernel/ipc-runtime/src/ipc/{mod.rs,endpoint_priority.rs}, kernel/compat/src/user/syscall/linux/{ipc_ops.rs,ipc_reply_diagnostics.rs}, services/{netd,inputd,devmgrd}/src/main.rs, services/rootd/src/{main.rs,control_drain.rs}, and services/runtimed/src/main.rs |
