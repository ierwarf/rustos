...
read-only media-barrier request traverses
`vfs-policy->block-broker->shared-ring->linux-dvm->media-barrier`. That terminal
is liveness evidence for the admitted data plane, not backing-image write
durability.
...
bind a live production `source` independently to one executable
`witness_source`; test-module extraction cannot silently replace the production
boundary being proved. The durability fault also requires the bounded
storage-DVM negative KVM gate. `recovery-scenarios.tsv` uses the same explicit
production/witness split and hashes both inputs into its evidence.
...
An exact PR or SMP-iteration pass may be reused for at most 24 hours. These
two profiles reciprocally share a completed TLC result only for a model selected
by both, because their exhaustive worker/fingerprint/seed policy is identical;
the non-TLC PR lanes and release seal remain distinct. `tlc_cache.py` rejects it
...
### Revoke and qualification evidence

`dvm-block-transport-revoked` is a Required milestone whose `arg0` is one of
the twelve production revoke reasons and whose `arg1` is the installed
generation. The terminal guard captures flags plus all four ring cursors once,
before clearing pending slots or readiness; a valid FLUSH completion must still
admit the first 64 KiB READ. Only the exact
`smp-qualification-{ready,start,finish,complete}` names are
QualificationCritical. Their rendered frames carry qualification-local loss
counters, a critical drop increments both global and local counters, scheduler
measurements make one attempt, and xtask rejects nonzero qualification loss.
...
| capability-derivation-lifecycle/CapabilityDerivationLifecycle | `kernel/object` identity/epoch vocabulary, IPC transfer descriptors, and generational endpoint/reply/process/lifecycle adapters | object slot generation, provider lease epoch, and derivation revoke epoch remain distinct; endpoint/reply, process-handle, and exec-lifecycle adapters decode only their established generations and never assert liveness or consumption; a child retains only a typed subset of its parent rights; provider rotation or parent revoke makes a child stale |
...
| cow-frame-lifecycle/CowFrameLifecycle | `kernel-ps` VMA fork hold, `kernel-mm` exact frame mappings, Linux fork, and exception write-fault dispatch | a child remains suspended until every private-anonymous alias is exact and the parent's writable translations are acknowledged; a present write copies or promotes only a sole anonymous mapping; file-private mappings always copy; unmap and rollback retain the old frame until exact mapping and TLB settlement |
| page-cache-lifecycle/PageCacheLifecycle | `pagerd` bounded file-page policy | exact backing generations have one load owner; private COW preserves the shared-clean source; dirty pages require exact writeback; provider restart seals the stale generation and quarantines live frames; reclaim releases only after exact TLB acknowledgement |
| pager-frame-grant-lifecycle/PagerFrameGrantLifecycle | `kernel-mm` user-pager frame custody | a zeroed physical frame is represented outside ring0 only by an opaque generation-bound handle; exact full-fault binding and subset rights consume it once, while stale, mismatched, or W+X grants fail closed |
| pager-fault-slot-lifecycle/PagerFaultSlotLifecycle | `kernel-ps` exception fault custody | a fixed all-atomic slot binds one exact request and pager endpoint; only a fused scheduler handoff may block it, and exactly one generation-bound reply claim or cancellation may settle it without exception-time allocation, process-state locking, or service lookup |
| pager-region-agreement/PagerRegionAgreement | ring0 VMA table and pagerd region table | two replicas of one address map apply a single shared split/trim/remove rule, so every address ring0 will still dispatch a fault for keeps a pager region behind it under partial unmap, `PROT_NONE`, and split-headroom refusal; under pressure a replica keeps more than ring0, never less |
| pager-vma-publication/PagerVmaPublication | `kernel-ps` fault-region publication | one bounded writer stamps exact process/MM/VMA generations into an all-atomic sequence record; exception readers take no process-state or scheduler lock and admit only a stable, permission-matching, non-revoked region |
...
| page-table-map-transaction/PageTableMapTransaction | `kernel-mm` intermediate table publication and leaf rollback | tables are published by CAS against a concurrent fault installer and retained on failure; a failed mapping reverses the exact leaf log, performs one shootdown, and only then frees leaf frames; every reachable table is claimed by exactly one ownership ledger |
...
| netd-deferred-reply/NetdDeferredReply | netd AF_UNIX deferred poll queue plus AF_INET provider-start gate | the global AF_UNIX reservation includes mutex-queued and worker-detached batches; admission rejects expired work, tightens one immutable caller end once, never refreshes it on detach/requeue, and each accepted request makes exactly one terminal reply attempt, including expiry and queue poison failure. AF_INET stream creation separately rechecks that same immutable end under the state lock before the provider starts; it does not replace the AF_UNIX broker route |
...
| cpu-online-lifecycle/CpuOnlineLifecycle | fixed low-memory AP trampoline, BSP-sealed MTRR/PAT/cache baseline, kernel HAL logical CPU/xAPIC/GDT/TSS state, kernel-ps syscall CPU-local state, and nucleus lockdep identity | before SIPI the BSP release-seals an exact MTRR/PAT/cache baseline; each reset AP enters CD=1/NW=0 no-fill mode, WBINVDs before mailbox/paging, restores and reads back the exact baseline, and only then enables cache/private readiness; the BSP serializes an RX trampoline and generation mailbox through bounded INIT-SIPI-SIPI and OnlineParked acknowledgement, then retires startup pages R/NX; every CPU has disjoint bootstrap/GDT/TSS/RSP0/IST and GS/syscall storage, while dense APIC identity selects distinct lockdep slots; failed CPUs own no dispatch authority |
...
| scheduler-cpu-ownership/SchedulerCpuOwnership | nucleus lockdep CPU/APIC guard ownership plus kernel-ps CPU-local current-task, transition-wake mailbox, and IRQ dispatch gates | pending acquisition units convert to held guards or cancellation in local-IRQ atomic transitions; total depth equals pending plus held units, and a published guard pins the exact task and CPU until same-CPU release; a blocked outgoing task may acquire one queued wake owner while assembly retains its stack, but cannot dispatch until commit and can be claimed only once afterward; accounting disagreement, owner loss/duplication, guarded dispatch, migration, cross-CPU release, and underflow fail closed |
...
| scheduler-active-balance/SchedulerActiveBalance | `kernel-ps` active balance | a per-source counter makes the first and each eighth loaded opportunity due without RTC residue; only a runnable queued Local(source) continuation without current/transition custody and with target affinity may move once through the exact target mailbox/request, drain, and dispatch chain; continuously eligible queued work eventually reaches the target under explicit action fairness |
| scheduler-cpu-distribution/SchedulerCpuDistribution | `kernel-ps` scheduler | each CPU owns its System/User burst, virtual timeline, and bounded handoff FIFO; two local System turns reserve that CPU's next ordinary User turn, ordinary User selection admits only a least-runtime fair peer, idle stealing transfers through single-owner mailbox custody, process-owned server boost targets the worker's actual CPU, repeated rehome generations coalesce by slot, and only System recovery has an absolute ready-age rail because User wall-clock guarantees require explicit bandwidth admission |
...
| ipc-fast-handoff/IpcFastHandoff | fixed kernel IPC frame, per-CPU direct/remote scheduler custody, and syscalld compact identity wrapper | one exact blocked receiver owns a bounded allocation-free request; successful same-CPU and cross-CPU transfers each publish one custody, while any failed admission rolls back completely before the ordinary bounded IPC path restarts. Exact caller process/task identity and scheduling-context custody survive request, reply, cancellation, timeout, and endpoint failure. |
| synchronous-ipc-handoff/SynchronousIpcHandoff | kernel IPC call/reply transfer and `kernel-ps` per-CPU scheduler handoff FIFO | a terminal reply atomically releases its donation, wakes the exact caller, and captures one `{slot, task_id, owner_generation, target_cpu}` token under Scheduler; after Scheduler drops, only that captured CPU may retain it while task ID, generation, CPU, runnability, and Local/RemoteQueued custody still match. Selection additionally requires exact Local custody; a live RemoteQueued head is consumed as non-authoritative unless its mailbox was drained first. Deduplication, in-place generation refresh, and the bounded burst/fairness turn are local to each CPU FIFO, so an old-CPU stale record may coexist with new-CPU custody but cannot dispatch there or suppress the fresh enqueue. |
| synchronous-ipc-handoff-concurrency/SynchronousIpcHandoffConcurrency | `kernel-ps` two-task reply publication across per-CPU handoff FIFOs | two independently serialized Scheduler reply wakes may both be Local or RemoteQueued and may publish concurrently to the same or different CPU FIFOs. Each task owns a separate Begin/Finish phase; independent generation, CPU, state, runnable, drain, running, and retirement races cannot redirect, duplicate, or dispatch either token, and retirement purges only that slot from every CPU FIFO. |
...
| dvm-network-ring/DvmNetworkRing | DVM network ivshmem mapper, Linux WB UIO adapter/Ethernet relay, netd substrate | only a host-validated fixed header on an exact prefetchable RustOS-WB atomic aperture installs; Linux data-plane readiness separately requires its WB mapping and atomic controls; DVM counters are bounded before either kernel cursor advances; malformed/forged RX work is rejected without delivery; DVM header mutation cannot alter installed bounds |
...
| dvm-block-transport/DvmBlockTransport | `driver-domain-protocol` ABI-v2 block transport, existing q35 AHCI read-only ATAPI frontend, kernel signed-epoch admission/rebind, Linux storage-DVM relay, storaged frontend, and xtask verifier | fixed request/completion queues remain bounded; records are launch-generation bound and contain no addresses; completions bind exact request identity. The private snapshot is copied, set to mode 0600, file-synced, and containing-directory-synced before aperture creation and signing. The DVM disk is `readonly=on` behind `ide-cd` on existing `ide.0`, never the unsupported read-only `ide-hd`, and supplies no impossible block-size override. Linux requires exact built-in `CONFIG_BLK_DEV_SR=y` for immutable ATAPI while signed modular `sd_mod` owns writable AHCI. The image is 2048-byte aligned; signed and host-verified logical/physical media geometry is exactly 2048/2048 while capacity remains in 512-byte protocol sectors, and this read-only topology advertises `FLUSH` only, never FUA. Its generation-bound media-barrier completion proves the exact VFS-to-Linux-DVM path is live; it does not claim backing-image write durability. Restart revokes queued authority and only a newer L0-signed epoch can be readmitted; xtask captures the exact generation returned by epoch rotation and accepts recovery only when rebound evidence and RustOS/DVM/READ_ONLY flags name that same successor generation |
...
| userspace-wait-set/WaitSetRegistry | vfsd persistent epoll-object and interest registry plus checkpoint batch restore | live objects and each object's interests stay within the ABI product bound; retired objects retain no interests; an accepted restore adds exactly the disjoint batch while preserving prior membership, and duplicate or over-capacity restore preserves the exact prior registry |
...
| ipc-reply-deadline/IpcReplyDeadline | kernel IPC runtime, compat deadline wait, and reply-bound AF_INET publication | the initial AF_INET stream descriptor binds only to its exact live reply and netd process owner, remains invisible through one-shot completion, and is returned on cancellation or endpoint failure. The caller installs exactly one fd only after the same deadline and exact v7 token/domain/type/protocol/fd/status-flag validation; a rejected bind reclaims the invisible descriptor while netd discards its initial token once. AF_UNIX remains on its generic broker route; every blocked control cycle carries a finite break and stale or late replies cannot revive authority |
...
| smp-ring3-qualification/SmpRing3Qualification | signed ordinary catalog, pending private reconcile, injector-owned KVM-private `smpqual`, kernel binding FSM, and xtask evidence parser | the admitted topology is exactly 1/2/4/8. Signed ordinary entries commit without touching private storage; private reconciliation is pending/retryable after that commit, with endpoint 0/`-ENOENT` classified as `EAGAIN`, only mounted-file `NotFound` absent, candidate errors preserving published ordinary entries, and 250 ms through 5 s retry bounds. The sole pre-UI exception is the UI server or the exact `qualification_contract_for_launch(entry)` result. The injector marks only the suspended exact executable, which is a signed early-system `smpqual.elf` allowlist member; qualification forces a DVM block provider whose QEMU drive, signed create/rotate header, readiness check, and Linux `BLKROGET` relay check all agree on `READ_ONLY`. Runtimed binds before activation and retires a failed launch. The kernel derives the qualified deferred authority, binds one live SESSIOND owner/epoch plus exact owner/target process-MM identities, and stamps the sole absolute deadline in the activation transaction. Every worker retains one dense CPU and distinct nonzero TID through Ready/Start/Finish; Complete is worker 0 only after every Finish and post-join. A binding-lock-held phase sample checks deadline, work, worker/TID/CPU, and FSM, then revalidates endpoint identity before output. Host evidence carries kernel-stamped PID/TID and encoded nonzero binding-id/exact work, but never kernel process/MM generations. The v6 success record names v5 as its predecessor and binds the formal seal, RustOS boot/runtime images, actual attached DVM block disk path/bytes/SHA-256, signed `system/boot/early-system.img#apps/smpqual/smpqual.elf` bytes/SHA-256, DVM kernel/rootfs, and private contract; prelaunch revalidates each artifact before unique non-overwriting publication. In the bounded smoke path, immutable host prelaunch hashing/copy completes before the bounded input relay, then both guests spawn, and only the subsequent boot timestamp/deadline consumes the guest/relay readiness budget; the interactive unbounded relay remains a distinct path. FNV detects transport corruption/interleaving only and is not authentication. Fairness means eventual local bounded phase delivery in this finite model, not a scheduler-rate claim. |
...
| scheduling-context-budget/SchedulingContextBudget | scheduler context/domain budget ledger, timeout disposition, and reply-scoped passive-server custody | available plus pending refill credit is conserved for every context and domain; only runnable, eligible work charges; every synchronous reply retains bounded exact custody; nested calls preserve the root context; the latest live worker token owns billing; exhaustion publishes its exact context/reply and one bounded throttle action; and terminal settlement restores or revokes custody exactly once |
...
## Shared-RAM cache and AP PAT bindings

`dvm-block-startup/DvmBlockStartup` binds the block aperture to an exact
prefetchable non-I/O shared-RAM BAR and WB mappings on RustOS and Linux; WC is
not a valid atomic-ring alias. `dvm-input-ring/DvmInputRing` binds its same
BAR class to the host's ordinary `MAP_SHARED` producer and RustOS WB consumer.
`dvm-network-ring/DvmNetworkRing` separately permits RustOS installation before
Linux starts, while requiring a prefetchable RustOS-WB atomic contract at
install and a Linux-WB atomic contract before any relay data-plane activity.
The global MMIO registry rejects mixed-cache physical overlap before either a
direct-map or high-window mapping can publish, including an interval straddling
the direct-map limit; it reserves registry and direct-map metadata before the
first mapping/PTE mutation. `cpu-online-lifecycle/CpuOnlineLifecycle` requires
the BSP to seal an exact MTRR/PAT/cache baseline before SIPI. Every reset AP
clears NW while retaining CD, WBINVDs before mailbox/paging, restores the
sealed baseline in no-fill mode, flushes TLB/cache state, and only enables
caching after exact MTRR/PAT readback before `OnlineParked` or private-ready.
Display pixel memory remains a separate write-mostly WC contract and is not
admitted as atomic shared control memory.

Permanent early-boot APIC MMIO is a separate direct-map-leaf UC retype before
APIC/PIC use; generic high-window maps reject below-limit physical addresses.
Raw direct-map cache inspection/update is whitelisted to kernel-mm, its paging
facade, and io-manager's ownership transaction. Display pixels remain on the
io-manager WC mapper and are not admitted as atomic shared RAM.
