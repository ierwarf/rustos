...
| boot-storage-handoff/BootStorageHandoff | hostd storage admission/supervision plus durable VFIO lease | whole-device and every partition must be idle before bounded host flush; host-driver and VFIO authority are exclusive; DVM launch requires a durable exact runtime record and live generation-bound aperture; readiness binds that exact generation; active recovery observes the exact QEMU process exit before aperture revoke and restores the host driver only after revoke; failure retains quarantine |
...
| zero-trust-subsystems.tsv | boot/image parsers, user memory, service authority, every DVM shared-memory consumer, every local control socket, network frames | every inventoried ingress has explicit shape, authority, lifecycle/revoke, registered model, and executable source evidence; new `dvm_*.rs` consumers or service socket listeners fail selftest until added |
...
| dvm-block-transport/DvmBlockTransport | `driver-domain-protocol` fixed block ABI; target storage-DVM relay and storaged frontend | fixed request/completion queues remain bounded; records are launch-generation bound and contain no addresses; completions bind exact request identity; reads cannot invent durability; successful FUA and FLUSH completions report the exact stable operation; restart revokes queued authority and stale completions cannot enter the replacement epoch |
| dvm-block-startup/DvmBlockStartup | `kernel/io-manager` readiness predicate, compat block waiter, storage-DVM publication, and storaged first generation query | initial provider-not-ready remains sleepable; readiness publication before check, during registration, or after sleep cannot be lost; only an observed exact ready generation permits volume use; timeout and revoke are explicit terminal outcomes and never select bootstrap storage |
