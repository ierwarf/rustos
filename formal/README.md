...
| dvm-atomic-scanout/DvmAtomicScanout | future explicit physical-AMD DRM/KMS relay mode | acceptance specification only: root-only exporter open and all three read-only imports must precede readiness; each immutable slot becomes a direct DMA-BUF framebuffer, the new front remains pinned, the old front is released only after the replacement page-flip fence, device-write DMA authority is absent, and offline revokes the pool. The old unreachable relay consumer was removed, so this model remains a failed implementation gate until a new explicit mode and hardware evidence exist. |
