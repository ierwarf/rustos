...
| scheduler-cpu-distribution/SchedulerCpuDistribution | `kernel-ps` scheduler | two consecutive System turns reserve the next ordinary User turn; ordinary User selection admits only a least-runtime fair peer, while a bounded, deduplicated latency FIFO cannot exceed its eight-pick burst; only System recovery has an absolute ready-age rail because User wall-clock guarantees require explicit bandwidth admission |
