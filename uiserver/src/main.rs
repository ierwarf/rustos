...
use app::{
    AppState, VisualUpdate, CURSOR_BLINK_INTERVAL, IDLE_SLEEP, INPUT_EVENT_BATCH,
    RUNTIME_POLL_SLEEP, TARGET_FRAME_INTERVAL,
};
...
fn run() -> Result<(), i32> {
    ...
    loop {
        let input =
            input_loop::process_pending_input(&mut state, runtime_fd.as_raw_fd(), &mut events)?;
        ...
        if pending_update.is_empty() {
            let sleep_deadline = next_runtime_poll.min(now + IDLE_SLEEP);
        ...
        if now < next_present_at {
            ...
            let sleep_deadline = next_present_at.min(next_runtime_poll);
            input_loop::sleep_until(sleep_deadline);
            continue;
        }
