...
pub fn bind(api_ptr: *const DriverKernelApiV1) -> Result<(), i32> {
    ...
    if api.register_display_framebuffer.is_none()
        || api.log.is_none()
        || api.pci_find_device.is_none()
        || api.pci_read_config_u32.is_none()
        || api.pci_write_config_u32.is_none()
        || api.pci_get_bar_info.is_none()
        || api.map_mmio.is_none()
        || api.read_boot_file.is_none()
        || api.query_boot_framebuffer.is_none()
    {
        return Err(-95);
    }
