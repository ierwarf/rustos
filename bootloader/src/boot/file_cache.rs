...
use crate::boot_info::{BootFileEntry, BootFileManifest, BOOT_FILE_MANIFEST_TRUNCATED};
use crate::debug;
...
const PRIORITY_BOOT_FILES: [PriorityBootFile; 3] = [
    ...
    PriorityBootFile::optional(
        "system/apps/uiserver/uiserver.elf",
        cstr16!("\\system\\apps\\uiserver\\uiserver.elf"),
    ),
];
...
const BOOT_FILE_LIST_PATHS: [&uefi::CStr16; 4] = [
    ...
    cstr16!("\\EFI\\BOOT\\RUSTOS.BOOT"),
];

pub fn snapshot_boot_volume() -> Result<BootFileManifest, BootError> {
...
impl PriorityBootFile {
    ...
    const fn optional(normalized_path: &'static str, uefi_path: &'static uefi::CStr16) -> Self {
        Self {
            normalized_path,
            uefi_path,
            required: false,
