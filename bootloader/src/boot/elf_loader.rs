...
const MIN_LOAD_ADDR: usize = 0x0010_0000; // 1 MiB
const MAX_LOAD_END_EXCLUSIVE: usize = 512 * 1024 * 1024 * 1024; // 512 GiB

pub fn load_elf_image(image: &[u8]) -> Result<(usize, usize), BootError> {
    let elf = ElfFile::new(image).map_err(BootError::InvalidElf)?;
...
fn load_segment(image: &[u8], ph: &ProgramHeader<'_>) -> Result<(), BootError> {
...
fn validate_entry(entry_point: usize) -> Result<(), BootError> {
    if entry_point < MIN_LOAD_ADDR {
        return Err(BootError::InvalidElf(
            "entry point is below minimum load address",
        ));
    }
    if entry_point >= MAX_LOAD_END_EXCLUSIVE {
        return Err(BootError::InvalidElf(
...
fn validated_segment_bounds(ph: &ProgramHeader<'_>) -> Result<(usize, usize), BootError> {
    ...
    if segment_end > MAX_LOAD_END_EXCLUSIVE {
        return Err(BootError::InvalidElf(
