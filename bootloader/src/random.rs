...
// Best-effort seed when firmware/CPU RNGs are unavailable. This preserves bootability,
// but it should not be treated as a cryptographic entropy source.
fn fallback_seed(front_addr: u64, front_size: u64, back_addr: u64, back_size: u64) -> [u8; 32] {
    let mut state = unsafe { _rdtsc() }
        ^ front_addr.rotate_left(7)
        ^ front_size.rotate_left(19)
        ^ back_addr.rotate_left(31)
        ^ back_size.rotate_left(43)
        ^ (&front_addr as *const _ as usize as u64);
