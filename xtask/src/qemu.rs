...
use crate::util::{copy_tree, create_temp_dir, env_string, read_trimmed, resolve_command_path};
...
impl RunSession {
    fn create() -> Result<Self> {
        ...
        let mirror_dir = temp_dir.join("image");
...
fn run_qemu_with_options(config: &Config, options: RunOptions) -> Result<()> {
    ...
    copy_tree(&config.image_dir, &session.mirror_dir)?;

    let mut profile_args = build_run_profile_args(&options, &session.mirror_dir)?;
...
fn build_run_profile_args(options: &RunOptions, mirror_dir: &Path) -> Result<Vec<OsString>> {
    ...
    match options.profile.as_str() {
        "default" => {
            ...
            } else if options.usb_input {
                args.push(OsString::from("-machine"));
                args.push(machine_option("pc", true));
            }
        }
        "g14" => {
            args.push(OsString::from("-drive"));
