...
pub(crate) fn run_cargo_kernel_rustc(
    ...
    cmd!(
        sh,
        "{cargo} rustc {zflags...} -p {package} --target {target} --release -- {rustc_args...}"
    )
    .run()?;
    Ok(())
}
...
pub(crate) fn run_cargo_kernel_check(config: &Config, package: &str) -> Result<()> {
    ...
    cmd!(
        sh,
        "{cargo} check {zflags...} -p {package} --target {target}"
    )
    .run()?;
    Ok(())
}
...
pub(crate) fn copy_tree(src: &Path, dst: &Path) -> Result<()> {
    ...
    for entry in fs::read_dir(src)? {
        ...
        if file_type.is_dir() {
            ...
            return Err(format!("symlink copy is not supported for {}", src_path.display()).into());
        }
    }
    Ok(())
}
