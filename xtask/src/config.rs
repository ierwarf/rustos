...
pub(crate) struct Config {
    ...
    pub(crate) win_user_object: PathBuf,
    pub(crate) win_user_source: PathBuf,
    pub(crate) win_user_asm_source: PathBuf,
    pub(crate) artifact_win_user_exe: PathBuf,
    pub(crate) image_win_user_exe: PathBuf,
    pub(crate) printf_demo_source: PathBuf,
    pub(crate) artifact_printf_demo_elf: PathBuf,
    pub(crate) image_printf_demo_elf: PathBuf,
...
impl Config {
    pub(crate) fn from_env() -> Result<Self> {
        ...
        let nasm = env_os("NASM").unwrap_or_else(|| OsString::from("nasm"));
        ...
        Ok(Self {
            ...
            source_efi: env_path("SOURCE_EFI").unwrap_or_else(|| {
                root_dir.join(format!("target/{target}/release/{bootloader_package}.efi"))
            }),
            artifact_boot_efi: env_path("ARTIFACT_BOOT_EFI")
                .unwrap_or_else(|| artifact_dir.join("boot/bootloader.efi")),
            prekernel_source: env_path("PREKERNEL_SOURCE").unwrap_or_else(|| {
                root_dir.join(format!(
                    "target/{kernel_target}/release/{prekernel_package}"
                ))
            }),
            ...
            kernel_source: env_path("KERNEL_SOURCE").unwrap_or_else(|| {
                root_dir.join(format!("target/{kernel_target}/release/{kernel_package}"))
            }),
            ...
            user_binary: env_path("USER_SOURCE").unwrap_or_else(|| {
                root_dir.join(format!("target/{kernel_target}/release/{user_elf_package}"))
            }),
            ...
            win_user_object: env_path("WIN_USER_OBJECT")
                .unwrap_or_else(|| user_build_dir.join("uiserver-win.obj")),
            win_user_source: env_path("WIN_USER_SOURCE")
                .unwrap_or_else(|| user_build_dir.join("UISERVER.EXE")),
            win_user_asm_source: env_path("WIN_USER_ASM_SOURCE")
                .unwrap_or_else(|| root_dir.join("uiserver/winmain.asm")),
            ...
            printf_demo_source: env_path("PRINTF_DEMO_SOURCE")
                .unwrap_or_else(|| root_dir.join("userdemo/printf_console.c")),
    ...
    pub(crate) fn kernel_release_deps_dir(&self) -> PathBuf {
        self.root_dir
            .join(format!("target/{}/release/deps", self.kernel_target))
    }
}
