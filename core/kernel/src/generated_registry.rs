...
use crate::user::runtime::{self, DesktopLaunchTarget, DesktopProgramRegistration, DesktopRuntimeError};
...
pub(crate) fn register_loadable_drivers() -> Result<usize, &'static str> {
    ...
    for (line_number, line) in contents.lines().enumerate() {
        ...
        let name = required_field(line, "name")
            .map_err(|error| leak_string(format_line_error(DRIVER_REGISTRY_PATH, line_number, error)))?;
        let class_name = required_field(line, "class")
            .map_err(|error| leak_string(format_line_error(DRIVER_REGISTRY_PATH, line_number, error)))?;
        let bus_name = required_field(line, "bus")
            .map_err(|error| leak_string(format_line_error(DRIVER_REGISTRY_PATH, line_number, error)))?;
        let priority = required_field(line, "priority")
            .map_err(|error| leak_string(format_line_error(DRIVER_REGISTRY_PATH, line_number, error)))?;
        let path = required_field(line, "path")
            .map_err(|error| leak_string(format_line_error(DRIVER_REGISTRY_PATH, line_number, error)))?;
...
pub(crate) fn register_desktop_programs() -> Result<(), DesktopRuntimeError> {
    let contents = read_registry_text(DESKTOP_REGISTRY_PATH)
        .map_err(|error| DesktopRuntimeError::Registry {
...
fn parse_desktop_registration(
    ...
    let display_name = required_field(line, "display_name").map_err(|error| {
        DesktopRuntimeError::Registry {
            path: DESKTOP_REGISTRY_PATH,
            error: leak_string(format_line_error(DESKTOP_REGISTRY_PATH, line_number, error)),
        }
    })?;
    let image_path = required_field(line, "image").map_err(|error| DesktopRuntimeError::Registry {
        path: DESKTOP_REGISTRY_PATH,
        error: leak_string(format_line_error(DESKTOP_REGISTRY_PATH, line_number, error)),
    })?;
    let exec_path = required_field(line, "exec").map_err(|error| DesktopRuntimeError::Registry {
        path: DESKTOP_REGISTRY_PATH,
        error: leak_string(format_line_error(DESKTOP_REGISTRY_PATH, line_number, error)),
    })?;
    ...
    let logical_admin = required_field(line, "logical_admin")
        .map_err(|error| DesktopRuntimeError::Registry {
            path: DESKTOP_REGISTRY_PATH,
            error: leak_string(format_line_error(DESKTOP_REGISTRY_PATH, line_number, error)),
        })?;
    let console_hosted = required_field(line, "console_hosted")
        .map_err(|error| DesktopRuntimeError::Registry {
            path: DESKTOP_REGISTRY_PATH,
            error: leak_string(format_line_error(DESKTOP_REGISTRY_PATH, line_number, error)),
        })?;
    ...
    let weight_micros = weight.parse::<u64>().map_err(|_| DesktopRuntimeError::Registry {
        path: DESKTOP_REGISTRY_PATH,
    ...
    let logical_admin = parse_registry_bool(logical_admin).map_err(|error| {
        DesktopRuntimeError::Registry {
            path: DESKTOP_REGISTRY_PATH,
            error: leak_string(format_line_error(DESKTOP_REGISTRY_PATH, line_number, error)),
        }
    })?;
    let console_hosted = parse_registry_bool(console_hosted).map_err(|error| {
        DesktopRuntimeError::Registry {
            path: DESKTOP_REGISTRY_PATH,
            error: leak_string(format_line_error(DESKTOP_REGISTRY_PATH, line_number, error)),
        }
    ...
    let mut registration =
        DesktopProgramRegistration::new(display_name, ExecutableImage::new(image_path), weight_micros)
...
fn read_registry_text(path: &'static str) -> Result<String, &'static str> {
    ...
    String::from_utf8(bytes).map_err(|_| leak_string(format!("registry is not valid UTF-8: {path}")))
}
