...
pub(super) fn load_elf(image: &[u8]) -> Result<LoadedProcessImage, ProcessLoadError> {
    ...
    {
        let interpreter_image = vfs::read_path_to_vec_for_kernel(interpreter_path)
...
pub(super) fn initialize_linux_user_stack(
    ...
    let stack_words =
        build_linux_initial_stack_words(image, &argv_ptrs, &env_ptrs, random_addr, execfn_addr);
