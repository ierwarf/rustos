...
pub(super) fn linux_runtime_access_allows_path(
    ...
    process_state
        ...
        .map(|profile| {
            profile.search_dirs().iter().any(|dir| {
                absolute_path == dir
...
mod tests {
    ...
    fn runtime_config_glob_matches_ld_so_conf_entries() {
        assert!(runtime_config_pattern_matches("*.conf", "x86_64-linux-gnu.conf"));
        assert!(runtime_config_pattern_matches("rustos-*.conf", "rustos-extra.conf"));
