...
pub(super) fn validate_module_image(
    ...
    crate::debug::println!("driver validate parse begin: path={}", image_path);
    let elf = ModuleElf::parse(image.as_ref())?;
    crate::debug::println!("driver validate parse done: path={}", image_path);
    crate::debug::println!("driver validate elf header begin: path={}", image_path);
    ...
    crate::debug::println!("driver validate abi detect begin: path={}", image_path);
    ...
    crate::debug::println!("driver validate abi detect done: path={}", image_path);
...
fn detect_module_abi(
    ...
    crate::debug::println!(
        "driver abi detect: ensure critical symbols begin path={}",
        image_path
    );
    ensure_unique_critical_symbols(elf)?;
    crate::debug::println!(
        "driver abi detect: ensure critical symbols done path={}",
        image_path
    );
    crate::debug::println!(
        "driver abi detect: rustos header lookup begin path={}",
        image_path
    );
    let rustos_header_symbol = find_symbol(elf, RUSTOS_DRIVER_HEADER_SYMBOL)?;
    crate::debug::println!(
        "driver abi detect: rustos header lookup done path={} found={}",
        image_path,
        rustos_header_symbol.is_some()
    );
    if rustos_header_symbol.is_some() {
        crate::debug::println!(
            "driver abi detect: validate rustos exports begin path={}",
            image_path
        );
        validate_module_exports(elf, rustos_header_symbol)?;
        crate::debug::println!(
            "driver abi detect: validate rustos exports done path={}",
            image_path
        );
        crate::debug::println!(
            "driver abi detect: extract rustos header begin path={}",
            image_path
        );
        let header = extract_module_header(elf)?;
        crate::debug::println!(
            "driver abi detect: extract rustos header done path={}",
            image_path
        );
    ...
    crate::debug::println!(
        "driver abi detect: linux init lookup begin path={}",
        image_path
    );
    ...
    crate::debug::println!(
        "driver abi detect: linux init lookup done path={}",
        image_path
    );
...
fn symbol_table_entries(elf: &ModuleElf<'_>) -> Result<Vec<ModuleSymbol>, &'static str> {
    crate::debug::println!("driver symtab: begin");
    let section =
        find_named_section(elf, ".symtab")?.ok_or("module ELF does not contain .symtab")?;
    ...
    crate::debug::println!(
        "driver symtab: raw entries={} offset={:#x} size={:#x} entsize={:#x}",
        entries.len(),
        section.offset,
        section.size,
        section.entry_size
    ...
    let entries = entries
        ...
        .collect::<Vec<_>>();
    crate::debug::println!("driver symtab: done parsed={}", entries.len());
    Ok(entries)
}
...
fn ensure_unique_symbol(
    ...
    crate::debug::println!("driver symbols: count begin name={}", name);
    if symbol_match_count(elf, name)? > 1 {
        return Err(duplicate_error);
    }
    crate::debug::println!("driver symbols: count done name={}", name);
    Ok(())
}
...
fn find_symbol<'a>(
    ...
    crate::debug::println!("driver symbols: find begin name={}", name);
    let entries = symbol_table_entries(elf)?;
    let string_table = symbol_string_table(elf)?;

    for (index, entry) in entries.into_iter().enumerate() {
        if symbol_name_from_table(string_table, &entry)? == name {
            crate::debug::println!("driver symbols: find done name={} found=true", name);
            return Ok(Some((index, entry)));
        }
    }

    crate::debug::println!("driver symbols: find done name={} found=false", name);
    Ok(None)
}
...
fn symbol_match_count(elf: &ModuleElf<'_>, name: &str) -> Result<usize, &'static str> {
    ...
    crate::debug::println!("driver symbols: count result name={} count={}", name, count);
    Ok(count)
}
...
pub(super) fn section_header_entries(
    ...
    crate::debug::println!("driver sections: begin");
    ...
    crate::debug::println!(
        "driver sections: raw count={} start={:#x} entry_size={:#x}",
        entries.len(),
        start,
        entry_size
    );
    let entries = entries
        ...
        .collect::<Vec<_>>();
    crate::debug::println!("driver sections: done parsed={}", entries.len());
    Ok(entries)
}
