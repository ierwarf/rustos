...
fn try_bind_interface_to_driver(
    ...
    let (interface_ptr, id_match) = {
        ...
        let Some(id_match) = match_interface_id(interface, unsafe { (*driver).id_table }) else {
            crate::debug::println!(
                "usb core no-match: driver={:#x} intf={:#x} vendor={:04x} product={:04x} class={:#x}/{:#x}/{:#x}",
                driver as usize,
                interface.ptr,
                interface.vendor_id,
                interface.product_id,
                interface.interface_class,
                interface.interface_sub_class,
    ...
    let status = unsafe {
        ...
        crate::debug::println!(
            "usb core probe begin: driver={:#x} intf={:#x} match={:#x} alt={:#x} eps={} ep={:#x} epdesc={:#x}/{:#x}/{:#x}/{:#x}",
            driver as usize,
            interface_ptr as usize,
            id_match as usize,
            alt as usize,
    ...
    crate::debug::println!(
        "usb core probe end: driver={:#x} intf={:#x} status={}",
        driver as usize,
        interface_ptr as usize,
        status,
    );
    ...
    crate::debug::println!(
        "usb core bind: driver={:#x} intf={:#x} class={:#x}/{:#x}/{:#x} vendor={:04x} product={:04x}",
        driver as usize,
        interface_ptr as usize,
        unsafe { (*interface_ptr).cur_altsetting.as_ref() }
            .map(|alt| alt.desc[5])
            .unwrap_or(0),
        unsafe { (*interface_ptr).cur_altsetting.as_ref() }
            .map(|alt| alt.desc[6])
        ...
        unsafe { (*interface_ptr).cur_altsetting.as_ref() }
            .map(|alt| alt.desc[7])
            .unwrap_or(0),
        interface_vendor_id(interface_ptr),
        interface_product_id(interface_ptr),
    );
    true
}
...
fn interface_vendor_id(interface_ptr: *mut LinuxCompatUsbInterface) -> u16 {
    ...
    let descriptor = unsafe { &(*(device as *mut LinuxCompatUsbDevice)).descriptor };
