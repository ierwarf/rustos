...
pub fn try_write_console(bytes: &[u8]) -> bool {
    write_emergency_console(bytes, EmergencyWriteMode::Forced)
}
...
pub(crate) unsafe extern "C" fn register_driver_framebuffer(
    ...
    if !backend::install_driver_framebuffer(boot_protocol::FramebufferInfo {
...
fn write_emergency_console(bytes: &[u8], mode: EmergencyWriteMode) -> bool {
    if bytes.is_empty() {
        return true;
    }
    ...
    if matches!(mode, EmergencyWriteMode::Live) && !console.live_enabled {
        return false;
    }
...
fn emergency_console_framebuffer_changed() {
    ...
    if console.live_enabled || console.history_count != 0 {
        let mode = if console.live_enabled {
            EmergencyWriteMode::Live
        } else {
            EmergencyWriteMode::Forced
        };
        let _ = console.render(mode);
    }
}
...
impl EmergencyConsole {
    ...
    fn render(&mut self, mode: EmergencyWriteMode) -> bool {
        if matches!(mode, EmergencyWriteMode::Live) && !self.live_enabled {
            return false;
        }
