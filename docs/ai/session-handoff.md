...
- Current bounded evidence: `cargo xtask check`, `cargo xtask build`, and the
  15-model `smp-iteration` formal seal passed. Two consecutive 4-vCPU
  storage-DVM 30-second runs passed after separating atomic startup-cohort
  custody from ordinary spawn handoffs.
- Deferred-activation authority consumption now executes between complete
  scheduler preflight and runnable publication while retaining the documented
  `ProcBrokerRegistry -> Scheduler` ownership pair. Registry guards are dropped
  before evidence output, and source contracts reject the old split-commit
  order. The next 8-vCPU run dispatched all first-cohort tasks and started
  netd, devmgrd, and inputd; this closes the prior activation blocker.
- That 8-vCPU run still failed the storage gate. Initd repeatedly retried
  runtimed and storaged because loaderd's executable-snapshot calls to vfsd
  timed out with errno 110. UI runtime readiness nevertheless reached true.
  Resume by tracing the bounded loaderd-to-vfsd snapshot transaction under
  8-vCPU contention; do not reopen the now-proven activation path without
