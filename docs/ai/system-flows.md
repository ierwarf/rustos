...
| `atomic-process-activation-batch` | bounded unique cohort → exact requester binding → complete capability and scheduler preflight → atomic runnable publication and one-shot authority consumption → FIFO cohort first turns before loader reply | `initd`, `loaderd`, `kernel-compat`, `kernel-ps` | complete batch activation, shape/requester/capability/scheduler rejection without partial publication, requester-exit revoke, exact first-turn prefix |
...
| `cpu-online-lifecycle` | admitted logical CPU generation → startup → private architecture state → scheduler substrate → dispatch admission, quarantine, failure, or invariant panic | `kernel-hal` | online success, startup failure, skipped/stale transition panic |
