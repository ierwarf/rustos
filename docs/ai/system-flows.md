...
| `capability-derivation-lifecycle` | nonzero typed object identity → generation-safe descriptor, process/lifecycle, and IPC endpoint/reply adapters → same-kind rights attenuation under exact lease/revoke epochs → internal IPC transfer identity bind → service revoke or stale-ticket rejection | `kernel/object`, IPC runtime, process table and transfer registry | in-range identity decoding without a liveness or consumption claim, attenuated derived authority, widening/epoch-drift denial, terminal revoke, stale ticket rejection |
...
| `page-cache-lifecycle` | exact backing-generation miss → one load owner or coalesced waiter → clean shared page → private COW or shared dirty → exact writeback → mapping revoke → exact TLB ACK → frame release | `pagerd` policy, `vfsd` open-description owner, `storaged` durability, `kernel-mm` PTE/TLB/frame substrate | cold owner/coalesced hit, private source isolation, stale backing revoke, dirty settlement, mapped/dirty reclaim rejection, exact ACK release |
| `pager-frame-grant-lifecycle` | normal-time VMA leaf-table preparation → boot-time bounded wired zeroed-frame reserve → exception-time lock/allocator-free opaque generation-bound grant publication → exact fault/process/MM/VMA/pager-epoch and rights revalidation → one-shot prepared-leaf PTE install with fixed ownership ledger, or exact cancellation/return-to-reserve | `kernel-mm`; `pagerd` may return only the opaque handle | consumed once, unprepared-leaf/stale/mismatched rejection, W+X rejection, reserve pressure/exhaustion failure, exact cancellation |
| `pager-fault-slot-lifecycle` | exception inputs → fixed all-atomic slot reservation → exact request/endpoint publication → fused scheduler `FaultPending`→`BlockedOnPager` commit → direct endpoint-exact pager-worker handoff with one-shot vruntime floor → token-bound scheduling-context donation → one worker-bound reply claim or cancellation → opaque-frame PTE commit → donation release before donor wake | `kernel-ps` owns slot and direct scheduler custody; `kernel-compat` binds/revokes donation and commits the exact frame; `pagerd` owns policy only | malformed/pressure rejection, stale token rejection, endpoint/worker mismatch rejection, reply-before-dispatch rejection, donation-admission rejection, duplicate reply rejection, cancellation wins, no exception-time allocation/lock/service lookup/generic IPC |
| `pager-vma-publication` | unbound region template → overlap/W^X/protection validation → kernel-stamped process/MM/VMA generations → even all-atomic commit → bounded exception lookup → exact-generation revoke | `kernel-ps`; `syscalld` supplies policy template; `pagerd` supplies object and endpoint authority | stable permission-matching snapshot, malformed/overlap/pressure rejection, stale exec identity rejection, revoked miss |
| `cow-frame-lifecycle` | all live VMAs held unstable → exact child shared aliases → parent read-only+COW downgrade → exact root TLB acknowledgement → child activation → present-write copy or anonymous sole-survivor promotion → exact unmap/retirement release | `kernel-ps` VMA/process transaction, `kernel-mm` PTE and frame descriptors, Linux fork broker | shared child, copied private writer, promoted anonymous survivor, rollback without alias, private-file no-promotion, no reuse before acknowledgement |
...
| `process-lifecycle-transaction` | non-reused spawn/exec/exit transaction → bounded unpublished frame acquisition/rollback → exact process and MM publication → attachment seal → authority settlement → parent-observed reap and generation retirement | `kernel-ps`, `kernel-mm` | running, rolled back, exit pending/exiting, reap queued/dead, stale token or attachment rejected |
