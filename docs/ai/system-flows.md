...
| `pager-fault-slot-lifecycle` | exception inputs → fixed all-atomic slot reservation → exact request/endpoint publication → fused scheduler `FaultPending`→`BlockedOnPager` handoff → normal-time dispatch with one opaque zeroed-frame grant → one generation-bound reply claim or cancellation → consume and release | `kernel-ps` owns slot custody; `pagerd` receives only the prebound endpoint request and its one-shot opaque grant | malformed/pressure rejection, stale token rejection, reply-before-block rejection, duplicate reply rejection, cancellation wins, no exception-time allocation/lock/service lookup |
