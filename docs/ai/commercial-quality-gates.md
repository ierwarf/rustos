...
| P0.3 | User memory, pager, service heaps, and page tables | Every user-copy and map operation proves canonical range, page rounding, access direction, non-overlap, backing lifetime, and teardown. A rejected `MAP_FIXED` request validates flags and backing before it may remove an existing VMA; multi-page protect/unmap validates the complete span and reserves ownership/region ledgers before changing the first PTE. Kernel mappings never follow guest pointers. Pagerd admits one load owner per exact backing-generation key, preserves the shared-clean source on private COW, requires exact writeback before dirty reclaim, permanently seals a revoked backing generation, quarantines its mapped or dirty frames, and cannot release a frame before exact TLB acknowledgement. A dirty quarantined frame moves to a newer backing generation only with the exact revoke token and unchanged object/page identity. Every long-lived service allocator returns dropped spans to reusable capacity, preserves alignment and exact ownership, installs bootstrap capacity once, rejects duplicate release without free-list mutation, and releases its spin lock before a blocking grow request; mmap hint exhaustion wraps to reusable holes. | `page-cache-lifecycle`, `service-heap-lifecycle`, page-table and process-address-space models; pagerd spec/implementation mutations; allocator reuse/coalescing/alignment/growth-lock tests; mmap plan, protection, and unmap preflight tests; source tests for `pagerd`, `kernel-mm`, user-copy, and brokers |
