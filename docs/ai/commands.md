...
| --- | --- | --- | --- |
| `cargo xtask verify` | **run the whole local pipeline as one command**: layering/manifest check, full build, host tests, formal selftest, then the `pr` seal - in that order, stopping at the first failure. Quiet on success: one line per stage with `elapsed_seconds`. A failing stage prints its stage name, exact rerun command, first relevant diagnostic, at most 120 trailing lines, and the full captured log at `build/verify/<stage>.log` | `target/`, `build/`, `build/formal/`, `build/verify/` on failure | whichever stage failed, named in its own bounded output |
...
| `cargo xtask bench` | boot the ordinary interactive topology and measure the full syscall/IPC probe table; compare only against a same-session control whose `vmexit_cpuid` anchor stays within 3% | `build/kvm/` | boot/readiness failure, missing `ipcbench: end`, or an unanchored comparison |
| `cargo xtask bench --isolate-probe <name>` | run one probe with the mid-boot phase-counter drain and verify once-per-call IPC attribution | `build/kvm/` | unknown probe, missing phase evidence, or isolation ratio outside 0.95..=1.05 |
| `RUSTOS_LOCK_PHASE_PROFILE=true cargo xtask bench --isolate-probe <name>` | diagnostic-only tracked-lock phase/class/site census; it intentionally preserves complete counters and materially inflates latency | `build/kvm/` | absent phase rows, failed isolation, or treating instrumented ticks as shipping latency |
...
| `bash formal/run-all-tlc.sh` | admit the 120-second, fail-closed PR set of critical TLA+ models at unchanged finite configurations, reusing only exact-input recent passes that retain the declared five-minute seal reserve; PR and SMP-iteration reciprocally reuse shared-model results under the identical execution policy, while every miss runs and `--profile nightly` runs the full registry without reuse | temporary files only | model failure, an invalid/expired/near-expiry cache followed by a failed rerun, a global wall-budget breach, or an unbounded/partial result |
| `bash formal/verify-all.sh --profile pr` | run the registered merge gate: selftest and exhaustive TLC first, then independent mutation, Loom/Shuttle/herd7, Kani/Verus, trace, dual-ABI, and recovery lanes concurrently with every child status collected before sealing; each lane and the sealed gate report `elapsed_seconds`, so the pole lane is readable from the output. Successful Kani package logs are content-addressed by the complete local Cargo dependency closure and proof/tool policy, so unrelated changes reuse them without weakening the fresh summary/seal requirement | `build/formal/`, tool caches including `build/formal/kani-cache-v1/` | invariant/proof/trace/reference/recovery/mutation failure, missing pinned tool, cache-key/input drift followed by a failed fresh proof, or any failed parallel lane |
...
| `bash formal/run-implementation-mutations.sh [--only <id> ...]` | seal each resolved source offset/context/hash in an isolated live-tree copy, prove each shard's pristine-tree preconditions once per Cargo selection (listing) and once per registered witness (baseline) before any mutant runs, then inject each registered critical/high regression and require that exact witness to kill it. PR reuse is keyed by the selected anchor context and literal exact-witness body, so an unrelated edit in a large scheduler file does not invalidate hundreds of decisions; nightly sets `RUSTOS_IMPLEMENTATION_MUTATION_CACHE=off` and re-executes the complete corpus | `build/formal/implementation-mutations/` | survived mutant, compile-only or foreign-target rejection, source-seal drift, ambiguous/missing witness, or an exact witness that did not execute |
