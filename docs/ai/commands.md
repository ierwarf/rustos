...
| `bash formal/run-all-tlc.sh` | admit the 120-second, fail-closed PR set of 21 critical TLA+ models at unchanged finite configurations, reusing only exact-input recent passes and running every miss; `--profile nightly` runs the full registry without reuse | temporary files only | model failure, an invalid/expired cache followed by a failed rerun, a global wall-budget breach, or an unbounded/partial result |
| `bash formal/verify-all.sh --profile pr` | run the registered merge gate: selftest, exhaustive TLC and mutation sensitivity, source-anchored Loom/Shuttle/herd7 concurrency triangle, witness-covered Kani/Verus, source trace replay, dual-ABI reference comparison, recovery scenarios, and implementation-mutation witnesses | `build/formal/`, tool caches | invariant/proof/trace/reference/recovery/mutation failure or missing pinned tool |
