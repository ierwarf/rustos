...
- `logs/`: host fuzz and local helper logs; KVM smoke logs are under `build/kvm/`
...
Do not put generated artifacts, KVM smoke output, or local debug captures in
source directories.
...
- `build/artifacts/`: build artifact cache
- `build/image/`: staged boot volume root
- `logs/`: host fuzz, local debug, helper log; KVM smoke log는 `build/kvm/`
...
생성 artifact, KVM smoke 실행 output, local debug capture를 source directory에
두지 마세요.
