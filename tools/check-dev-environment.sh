...
require_command() { if command -v "$1" >/dev/null 2>&1; then ok "$1 available"; else bad "$1 missing"; fi; }
...
cd -- "$ROOT"
for command in bash cargo git jq make rustc rustup sha256sum; do require_command "$command"; done
...
if jq -s -e '
    .[0] as $launch | .[1] as $settings |
    $launch.version == "0.2.0" and ($launch.configurations | type) == "array" and
    ($launch.configurations | length) == 1 and
    ($launch.configurations[0] | .name == "RustOS: verified KVM desktop" and
      .type == "node-terminal" and .request == "launch" and
      .command == "exec cargo xtask kvm-run --build --rustos-vcpus 8" and
      .cwd == "${workspaceFolder}") and
    ($launch.configurations[0] | has("preLaunchTask") | not) and
    ([$launch | .. | strings | select(. == "build-dvm")] | length) == 0 and
    ($settings | type) == "object"
  ' .vscode/launch.json .vscode/settings.json >/dev/null && test ! -e .vscode/tasks.json; then
...
if test "$CHECK_DOCS" -eq 1; then
    ...
    case "$mdbook_version" in *'0.4.52'*) ok "mdBook version is 0.4.52" ;; *) bad "mdBook version is not 0.4.52: $mdbook_version" ;; esac
fi

if test "$CHECK_FORMAL" -eq 1; then
    for command in clang curl pkg-config python3 timeout; do require_command "$command"; done
    require_command java
    java_major=$(java -version 2>&1 | sed -n '1s/.*version "\([0-9]*\).*/\1/p')
    if test -n "$java_major" && test "$java_major" -ge 17; then ok "Java major version is $java_major"; else bad "Java 17 or newer is required"; fi
    if bash formal/selftest.sh >/dev/null; then ok "formal model registry is internally consistent"; else bad "formal model registry selftest failed"; fi
fi

if test "$CHECK_PHYSICAL" -eq 1; then
    require_command qemu-system-x86_64
    qemu_major=$(qemu-system-x86_64 --version 2>/dev/null | sed -n '1s/.*version \([0-9]*\).*/\1/p')
    if test -n "$qemu_major" && test "$qemu_major" -ge 11; then ok "QEMU major version is $qemu_major"; else bad "QEMU 11 or newer is required"; fi
    test -c /dev/kvm && test -r /dev/kvm && test -w /dev/kvm && ok "/dev/kvm is usable" || bad "/dev/kvm is not directly usable"
    test -c /dev/iommu && test -r /dev/iommu && test -w /dev/iommu && ok "/dev/iommu is usable" || bad "/dev/iommu is not directly usable"
    test -d /dev/vfio/devices && ok "/dev/vfio/devices exists" || bad "/dev/vfio/devices missing"
fi

if test "$CHECK_RELEASE" -eq 1; then
    require_command gpg
    test -n "${RUSTOS_GRUB_SIGNING_KEY:-}" && ok "RUSTOS_GRUB_SIGNING_KEY is set" || bad "RUSTOS_GRUB_SIGNING_KEY is unset"
    test -n "${RUSTOS_GPG_HOME:-}" && test -d "${RUSTOS_GPG_HOME:-}" && ok "RUSTOS_GPG_HOME exists" || bad "RUSTOS_GPG_HOME is absent or invalid"
fi

if test "$failures" -ne 0; then printf 'check-dev-environment: %d failure(s)\n' "$failures" >&2; exit 1; fi
