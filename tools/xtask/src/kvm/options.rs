    ...
    I: Iterator<Item = String>,
{
    ...
    // code that is no longer in the tree. Whole sessions have been spent
    // "rejecting" hypotheses that were never actually booted. A no-op build is
    // about two seconds next to a thirty-second boot, so pay it every time
    // unless the caller explicitly wants the artifact already on disk.
    ...
    repeat_smoke(config, &options, runtime_trace_deadlines)
}

/// Boots the same topology `options.repeat` times and reports the pass rate.
///
/// Hand-rolling this loop in a shell loses the failing run's evidence to the
/// next run's truncation, and loses the panic site with it. Each run's archive
/// is named here, and a run that panicked is distinguished from a run that
...
fn repeat_smoke(
    ...
    let runs = options.repeat;
    let mut failures: Vec<(usize, String)> = Vec::new();
    for index in 1..=runs {
        let before = crate::soak::newest_archive(&history);
        let outcome = run_one_smoke(config, options, runtime_trace_deadlines);
        match outcome {
            Ok(()) => println!("xtask: smoke run {index}/{runs} passed"),
            Err(error) => {
                let archived =
                    crate::soak::newest_archive(&history).filter(|path| Some(path) != before.as_ref());
                let reason = match archived.as_deref().and_then(crate::soak::guest_panic) {
                    Some(panic) => format!("guest panic: {panic}"),
                    None => format!("{error:#}"),
                };
                let archive = archived.map_or_else(
                    || "no archived log".to_owned(),
                    |path| path.display().to_string(),
                );
                eprintln!("xtask: smoke run {index}/{runs} failed: {reason} [{archive}]");
                failures.push((index, format!("{reason} [{archive}]")));
            }
        }
    }
    let passed = runs - failures.len();
    println!(
        "xtask: smoke repeat complete passed={passed}/{runs} vcpus={}",
        options.rustos_vcpus
    );
    for (index, reason) in &failures {
        println!("  run {index}: {reason}");
    }
    if failures.is_empty() {
        Ok(())
    } else {
        bail!("{} of {runs} smoke runs failed", failures.len())
    }
}
