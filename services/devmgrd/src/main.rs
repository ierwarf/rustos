...
use rustos_user_abi::syscall::{
    ...
    DevmgrdNodeEntry, IpcReplyWithHandlesArgs, RustosDeviceIoctlBrokerArgs,
    RustosDeviceOpenBrokerArgs, RustosIpcValidateServiceOwnerArgs,
    ...
    DEVMGRD_NODE_KIND_DIR, IPC_ABI_VERSION, IPC_MAX_INLINE_BYTES, IPC_SERVICE_DEVMGRD,
    ...
    SYS_RUSTOS_IPC_RECV_WITH_SENDER, SYS_RUSTOS_IPC_REPLY, SYS_RUSTOS_IPC_REPLY_WITH_HANDLES,
    ...
    loop {
        ...
        let reply = match received as usize {
            ...
            size if size == size_of::<DevmgrdDeviceOpenRequest>() => {
                ...
                reply_device_open(reply_cap, &request, sender_pid, sender_tid)
            }
            size if size == size_of::<DevmgrdDeviceIoctlRequest>() => {
                ...
                if !identity_is_exact_sender(request.pid, request.tid, sender_pid, sender_tid) {
                    reply_device_ioctl_status(reply_cap, libc::EACCES)
                } else if request.op == DEVMGRD_IPC_OP_IOCTL_AUTHORIZE
            ...
            size if size == size_of::<CommercialMaxProtocolRequest>() => {
                ...
                reply_commercial_request(reply_cap, &request, sender_pid, sender_tid)
            }
            _ => {
...
fn reply_device_open(
    ...
    response.status = match validate_device_open_request(request) {
        Ok(_) if !identity_is_exact_sender(request.pid, request.tid, sender_pid, sender_tid) => {
            libc::EACCES
        }
        Ok(policy) => {
...
fn authorize_ioctl_request(request: &DevmgrdDeviceIoctlRequest) -> Result<(), i32> {
    ...
    match ioctl_policy_owner(request.request) {
        ...
        Some(IoctlPolicyOwner::Uiserver(op)) => {
            // Uiserver is the sole display-policy owner. Re-entering its own
            // receive endpoint for metadata/atlas setup creates a synchronous
            // uiserver -> devmgrd -> uiserver dependency and can hold GPU
            // bootstrap behind an unrelated policy-worker scheduling turn.
            // The kernel-owned service registry is the authority: only the
            // exact current uiserver publication owner gets this delegated
            // self path; every other process still crosses the policy owner.
            if process_owns_live_service(request.pid, IPC_SERVICE_UISERVER) {
                Ok(())
            } else {
                authorize_uiserver_ioctl(op, request.request)
            }
        }
...
fn process_owns_live_service(process_id: u64, service_id: u64) -> bool {
...
fn authorize_session_ioctl(op: u16, request_number: u64) -> Result<(), i32> {
    ...
    request.header.protocol = COMMERCIAL_MAX_PROTOCOL_SESSIOND;
    request.header.op = op;
    stamp_current_subject(&mut request)?;
...
fn stamp_current_subject(request: &mut CommercialMaxProtocolRequest) -> Result<(), i32> {
