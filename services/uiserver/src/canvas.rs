...
impl<'a> SurfaceCanvas<'a> {
    ...
    pub(crate) fn fill_radial_glow_cooperative(
        &mut self,
        center_x: i32,
        center_y: i32,
        radius: u32,
        color: u32,
        max_alpha: u8,
        yield_every_rows: usize,
    ) {
        if radius == 0 || max_alpha == 0 || yield_every_rows == 0 {
            return;
        }
        ...
        for y in bounding.y..bounding.y.saturating_add(bounding.height) {
            // The Aurora composite is generated by a background uiserver
            // thread on the same vCPU as the interactive loop. Bound each
            // compute slice so cold-start decoration cannot monopolize the
            // process long enough to turn pointer updates into visible
            // stalls. `sched_yield` is backed by the one-shot user-return
            // reschedule tick, so this does not schedule from a live syscall
            // continuation.
            if y != bounding.y && (y - bounding.y).is_multiple_of(yield_every_rows) {
