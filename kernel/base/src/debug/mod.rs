...
#[cfg(rustos_debug_print_enabled)]
use diag_abi::{
    CrashStoreHeader, CrashStoreInfo, DiagLevel, DiagProvider, DiagSharedBufferHeader, DiagStage,
};
...
fn current_diag_subject_ids() -> (u64, u64) {
    (
        crate::multitask::current_user_process_id().unwrap_or(0),
        crate::multitask::current_user_id().unwrap_or(0),
    )
}
...
pub fn capture_crash_snapshot(panic_text: &str) {
    with_diag_state_lock(|state| {
        ...
        let records_ptr =
            (state.crash_store.addr as usize + core::mem::size_of::<CrashStoreHeader>())
                as *mut DiagRecord;
...
#[cfg(rustos_debug_print_enabled)]
#[allow(dead_code)]
pub(crate) fn write_debugcon_only(bytes: &[u8]) {
    if bytes.is_empty() {
        return;
    }
    with_debug_output_lock(|| {
...
#[cfg(not(rustos_debug_print_enabled))]
#[allow(dead_code)]
pub(crate) fn write_debugcon_only(_bytes: &[u8]) {}

#[cfg(rustos_debug_print_enabled)]
#[allow(dead_code)]
pub(crate) fn write_debugcon_only_line(bytes: &[u8]) {
    with_debug_output_lock(|| {
        if !bytes.is_empty() {
            print_bytes_unlocked(bytes);
        }
...
#[cfg(not(rustos_debug_print_enabled))]
#[allow(dead_code)]
pub(crate) fn write_debugcon_only_line(_bytes: &[u8]) {}
...
fn write_debug_bytes(bytes: &[u8]) {
    // Keep the default debug path side-effect free with respect to the
    // interactive console and GUI state. Observation should go to debugcon
    // and the structured diag ring only unless a caller opts into another
    // output plane explicitly.
    with_debug_output_lock(|| {
        print_bytes_unlocked(bytes);
    });
}
