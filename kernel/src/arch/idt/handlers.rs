...
fn log_general_protection_details(error_code: u64, rip: u64, rsp: u64) {
    let rip_flags = crate::memory::kernel_vm::debug_direct_map_flags_for_addr(rip);
    crate::debug::println!(
        "general protection detail: ec={:#x} rip_flags={:?}",
        error_code,
        rip_flags.map(|flags| flags.bits()),
    );
...
fn log_page_fault_details(error_code: u64, cr2: u64, rip: u64, rsp: u64) {
    ...
    let rip_flags = crate::memory::kernel_vm::debug_direct_map_flags_for_addr(rip);
    let cr2_flags = crate::memory::kernel_vm::debug_direct_map_flags_for_addr(cr2);

    crate::debug::println!(
        "page fault detail: ec={:#x} present={} write={} user={} rsvd={} ifetch={} pkey={} sstk={} sgx={} rip_flags={:?} cr2_flags={:?}",
        error_code,
        present,
        write,
        user,
        ...
        sgx,
        rip_flags.map(|flags| flags.bits()),
        cr2_flags.map(|flags| flags.bits()),
    );
