    ...
    match event {
        XhciEvent::Transfer {
            ...
            let mut should_resubmit = true;
            {
                ...
                let report_len = if completion_code == XHCI_COMP_SUCCESS
                    || completion_code == XHCI_COMP_SHORT_PACKET
                {
                    device
                        .endpoint_max_packet_size
                        .saturating_sub(residual_len as u16) as usize
                } else {
                    0
                };

                if report_len != 0 && report_len <= device.poll_buffer.size {
                    let bytes = unsafe {
                ...
                {
                    crate::debug::println!(
                        "xhci transfer error: slot={} ep_id={} code={} residual={}",
                        slot_id,
                        ep_id,
                        completion_code,
                        residual_len
...
fn submit_interrupt_poll(
    ...
    let trb_dma = enqueue_trb(&mut device.interrupt_ring, trb.into_raw());
...
fn ring_doorbell(controller: &mut XhciControllerState, slot_id: u8, target: usize) {
    ...
    doorbell.set_doorbell_target(target as u8);
