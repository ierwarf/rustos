...
use crate::user::abi::ui::{
    ...
    INPUT_KIND_POINTER_SCROLL, POINTER_BUTTON_LEFT, UiInputEvent,
};
...
pub(crate) fn push_keyboard_event(event: KeyboardEvent) {
    with_event_queue(|events| {
        events.push_overwrite(UiInputEvent {
...
pub(crate) fn read_input_events(dest: &mut [UiInputEvent]) -> usize {
...
fn with_event_queue<R>(
    f: impl FnOnce(&mut RingBuffer<UiInputEvent, INPUT_EVENT_QUEUE_CAPACITY>) -> R,
) -> R {
    #[cfg(test)]
    {
