...
pub(crate) fn debug_snapshot() -> InputEventQueueDebugSnapshot {
    with_event_queue(|events| InputEventQueueDebugSnapshot {
        ...
        queued: events.queued.len(),
        pending_coalesced: events.pending_coalesced.is_some(),
        pending_pointer_position: events.pending_pointer_position.is_some(),
        dropped_discrete: events.dropped_discrete_events,
        dropped_lossy: events.dropped_lossy_events,
    })
}
