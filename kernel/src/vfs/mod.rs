...
pub(crate) fn init() {
    ...
    crate::debug::println!("vfs: register filesystem runfs");
    register_filesystem(&runfs::RUNFS_PROVIDER);
    ...
    let _ = mount_internal("/run", "runfs", MountSource::None, 0, None, true);
...
pub(crate) fn open_path_for_current_process(
    ...
    if !mount.backend.requires_live_process_state() {
        let opened = with_user_backend_context(&mount, absolute_path, false, |context| {
            mount.backend.open(
                absolute_path,
    ...
    let opened = with_user_backend_context(&mount, absolute_path, true, |context| {
        mount.backend.open(
            absolute_path,
            mount.relative_path.as_str(),
            flags,
...
pub(crate) fn metadata_for_current_process_path(
    ...
    let mount = resolve_mount(absolute_path)?;
    if !mount.backend.requires_live_process_state() {
        return with_user_backend_context(&mount, absolute_path, false, |context| {
            mount
                .backend
                .metadata(absolute_path, mount.relative_path.as_str(), context)
        });
    }

    with_user_backend_context(&mount, absolute_path, true, |context| {
        mount
            .backend
            .metadata(absolute_path, mount.relative_path.as_str(), context)
    })
...
pub(crate) fn read_path_to_vec_for_kernel(absolute_path: &str) -> Result<Vec<u8>, VfsError> {
    const KERNEL_READ_CHUNK_CAP: usize = 64 * 1024;
    ...
    match mount.backend.open(
        ...
        VfsOpenResult::File(file) => {
            ...
            let mut chunk = vec![0_u8; file_len.min(KERNEL_READ_CHUNK_CAP)];
            let mut copied = 0usize;
            while copied < file_len {
                let read_len = chunk.len().min(file_len - copied);
                let read = file.read_at(copied, &mut chunk[..read_len]);
                if read == 0 {
                    crate::debug::println!(
                        "vfs: short kernel read path={} copied={}/{}",
                        absolute_path,
                        copied,
                        file_len
                    );
...
fn normalize_absolute_kernel_path(path: &str) -> Result<String, VfsError> {
    if !path.starts_with('/') {
        return Err(VfsError::InvalidArgument);
    }
...
pub(crate) fn check_access_for_current_process(
    ...
    if !mount.backend.requires_live_process_state() {
        return with_user_backend_context(&mount, absolute_path, false, |context| {
            mount
                .backend
                .check_access(absolute_path, mount.relative_path.as_str(), mode, context)
        });
    }
...
pub(crate) fn readlink_for_current_process(absolute_path: &str) -> Result<String, VfsError> {
    if absolute_path.starts_with("/dev/fd/") {
        return with_current_user_shared_context(|context| {
            procfs::read_fd_link(absolute_path, context)
        });
    }
    ...
    if !mount.backend.requires_live_process_state() {
        return with_user_backend_context(&mount, absolute_path, false, |context| {
            mount
                .backend
                .readlink(absolute_path, mount.relative_path.as_str(), context)
        });
    }
...
pub(crate) struct VfsContext<'a> {
    abi: Option<UserAbi>,
    process_state: Option<&'a UserProcessState>,
}

impl<'a> VfsContext<'a> {
    pub(crate) fn new_user(abi: UserAbi, process_state: &'a UserProcessState) -> Self {
        Self {
            abi: Some(abi),
    ...
    pub(crate) const fn new_user_abi(abi: UserAbi) -> Self {
        Self {
            abi: Some(abi),
            process_state: None,
        }
...
fn with_current_user_shared_context<R>(
    f: impl FnOnce(&mut VfsContext<'_>) -> Result<R, VfsError>,
) -> Result<R, VfsError> {
    let retained = multitask::retain_current_user_process_state().ok_or(VfsError::Unsupported)?;
    let mut context = VfsContext::new_user(retained.abi(), retained.process_state());
    f(&mut context)
}

fn with_user_backend_context<R>(
    mount: &ResolvedMount,
    absolute_path: &str,
    include_process_state: bool,
    f: impl FnOnce(&mut VfsContext<'_>) -> Result<R, VfsError>,
) -> Result<R, VfsError> {
    let retained = multitask::retain_current_user_process_state().ok_or(VfsError::Unsupported)?;
    let abi = retained.abi();
    let process_state = retained.process_state();
    ensure_user_mount_access(mount, absolute_path, abi, process_state)?;
    let mut context = if include_process_state {
        VfsContext::new_user(abi, process_state)
    } else {
        VfsContext::new_user_abi(abi)
    };
    f(&mut context)
}
