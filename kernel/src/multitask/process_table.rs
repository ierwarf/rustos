...
impl ProcessRef {
    ...
    pub fn into_mm(self) -> MmRef {
        MmRef {
            handle: MmHandle {
                process: self.handle,
                generation: self.mm_generation,
            },
            process: self,
        }
    }
...
impl MmRef {
    pub const fn handle(&self) -> MmHandle {
        self.handle
    }
...
impl ProcessObject {
    fn new(process_id: u64, state: UserProcessState) -> Self {
        Self {
            process_id,
            ref_count: 1,
            thread_count: 1,
    ...
    fn mm_handle(&self, handle: ProcessHandle) -> MmHandle {
        MmHandle {
            process: handle,
            generation: self.mm_generation,
        }
...
impl ProcessTable {
    ...
    fn enqueue_reap(&mut self, handle: ProcessHandle, object: &mut ProcessObject) {
        if object.queued_for_reap {
            return;
        }
        object.queued_for_reap = true;
        assert!(
            self.reap_len < self.reap_queue.len(),
            "process reap queue overflow"
        );
...
pub fn create_process(process_id: u64, state: UserProcessState) -> Option<ProcessHandle> {
    ...
    let object = Box::new(ProcessObject::new(process_id, state));
...
pub fn attach_task(handle: ProcessHandle) -> Option<()> {
    ...
    object.ref_count = object
        .ref_count
        .checked_add(1)
        .expect("process task refcount overflow");
    object.thread_count = object
        .thread_count
        .checked_add(1)
        .expect("process thread count overflow");
...
pub fn detach_task(handle: ProcessHandle) -> Option<()> {
    ...
    if should_queue {
        assert!(
            table.reap_len < table.reap_queue.len(),
            "process reap queue overflow"
        );
        let queue_index = table.reap_len;
        table.reap_queue[queue_index] = Some(handle);
        table.reap_len += 1;
    }
    Some(())
}
...
pub fn retain_process(handle: ProcessHandle) -> Option<ProcessRef> {
    ...
    object.ref_count = object
        .ref_count
        .checked_add(1)
        .expect("process refcount overflow");
...
pub fn retain_mm(handle: ProcessHandle) -> Option<MmRef> {
    let mut table = PROCESS_TABLE.lock();
    let object = table.lookup_object_mut(handle)?;
    object.ref_count = object
        .ref_count
    ...
    let process_id = object.process_id;
    let state_ptr = object.state_ptr();
    let mm_handle = object.mm_handle(handle);
    Some(MmRef {
        process: ProcessRef {
            handle,
            process_id,
...
pub fn process_state_ptr(handle: ProcessHandle) -> Option<NonNull<UserProcessState>> {
    let table = PROCESS_TABLE.lock();
    Some(table.lookup_object(handle)?.state_ptr())
}

pub fn mm_handle(handle: ProcessHandle) -> Option<MmHandle> {
    let table = PROCESS_TABLE.lock();
    Some(table.lookup_object(handle)?.mm_handle(handle))
}
...

    {
        ...
        for handle in queue.into_iter().take(queue_len).flatten() {
            ...
            let object = slot
                .object
                .take()
                .expect("process object disappeared during reap");
...
mod tests {
    ...
    fn new_state() -> UserProcessState {
        UserProcessState::new(
            ProcessAddressSpace::new().expect("process address space"),
            None,
            None,
            None,
            None,
