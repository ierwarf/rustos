...
const SWITCH_TRACE_BUDGET: usize = 0;

static SWITCH_TRACE_COUNT: AtomicUsize = AtomicUsize::new(0);
...
impl Scheduler {
    ...
    pub(super) fn allocate_user_slot(
        ...
        for slot in 1..MAX_TASK {
            if self.contexts[slot].is_none() {
                ...
                let process_handle = process_table::create_process(id, boxed_state)?;
                let kernel_stack_top = self.stack_top(slot) as u64;
                debug::println!(
    ...
    fn trace_switch(&self, from_slot: usize, to_slot: usize) {
        ...
        let trace_index = SWITCH_TRACE_COUNT.fetch_add(1, Ordering::Relaxed);
        if trace_index >= SWITCH_TRACE_BUDGET {
            return;
        }
        ...
        debug::println!(
            "scheduler switch[{}]: slot {} ({:?}, user={}, rip={:#x}, rsp={:#x}) -> slot {} ({:?}, user={}, rip={:#x}, rsp={:#x})",
            trace_index,
            from_slot,
            from_id,
            from_user,
    ...
    pub(super) fn current_user_address_space(&self) -> Option<&ProcessAddressSpace> {
        ...
        let process_state = process_table::process_state_ptr(process_handle)?;
        let address_space =
            unsafe { process_state.as_ref().address_space() as *const ProcessAddressSpace };
        Some(unsafe { &*address_space })
    }
    ...
    pub(super) fn with_current_user_process_state_mut<R>(
        ...
        let process_handle = context.process_handle?;
        process_table::with_process_state_mut(process_handle, |_, process_state| {
            f(id, abi, process_state)
        })
    }

    pub(super) fn with_current_user_process_state<R>(
        &self,
        ...
        let id = self.starts[slot].map(|start| start.id)?;
        let process_handle = context.process_handle?;
        process_table::with_process_state(process_handle, |_, process_state| {
            f(id, abi, process_state)
        })
    }

    pub(super) fn retain_current_user_process_state(
        &mut self,
    ) -> Option<(u64, UserAbi, process_table::ProcessRef)> {
        let slot = self.current_task;
        let context = self.contexts[slot].as_mut()?;
        if !context.user_mode {
            return None;
    ...
    fn test_process(id: u64) -> process_table::ProcessHandle {
        process_table::create_process(
            ...
            UserProcessState::new(
                ProcessAddressSpace::new().expect("address space"),
                None,
                None,
                None,
                None,
