...
const OPENAT_TRACE_LIMIT: usize = 0;
static OPENAT_TRACE_COUNT: AtomicUsize = AtomicUsize::new(0);
...
pub(crate) fn openat(
    ...
    let trace_index = OPENAT_TRACE_COUNT.fetch_add(1, Ordering::Relaxed);
    let path = usermem::read_current_user_c_string(path_ptr, 128)?;
    if trace_index < OPENAT_TRACE_LIMIT {
        debug::println!(
    ...
    if trace_index < OPENAT_TRACE_LIMIT {
        debug::println!("linux openat[{}] end fd={}", trace_index, fd);
    }
    Ok(fd)
}
    ...
    match handle {
        ...
        KernelHandle::Device(handle) => match handle.device_id() {
            device_ns::DeviceId::Input => HandleReadiness {
                readable: event_queue::has_pending_input_events(),
                priority: event_queue::has_pending_input_events(),
                ..HandleReadiness::default()
            },
            device_ns::DeviceId::Console
            | device_ns::DeviceId::Display
            | device_ns::DeviceId::Runtime => HandleReadiness {
                writable: true,
                ..HandleReadiness::default()
            },
        },
