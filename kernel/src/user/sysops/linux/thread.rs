...
pub(crate) fn get_robust_list(
    pid: u64,
    head_ptr_ptr: u64,
    len_ptr: u64,
) -> Result<(), LinuxSysopError> {
    if pid != 0 && Some(pid) != multitask::current_user_id() {
        return if multitask::is_user_task_alive(pid) {
    ...
    let Some(result) =
        multitask::with_current_user_linux_state_mut(|_, _, abi, _, _, linux_thread_state| {
            ...
            usermem::write_current_user_struct(head_ptr_ptr, &state.robust_list_head)?;
            usermem::write_current_user_struct(len_ptr, &state.robust_list_len)?;
            Ok(())
        })
    else {
        return Err(LinuxSysopError::Unsupported);
