...
pub(crate) fn read_current_process_handle(
    ...
    if let Some(KernelHandle::Device(device_handle)) = handle {
    ...
    let Some(result) = multitask::with_current_user_process_state_mut(|_, _, process_state| {
        match process_state.handles().get(fd) {
            ...
            Some(KernelHandle::Epoll(_)) => {
                crate::debug::println!(
                    "device read wrong-handle: fd={} handle=epoll user_ptr={:#x} len={}",
                    fd,
                    user_ptr,
                    user_len,
                );
...
pub(crate) fn ioctl_current_process_device_handle(
    ...
    if device_handle.device_id() == device_ns::DeviceId::Runtime {
        let Some(process_state) = multitask::retain_current_user_process_state() else {
            return Err(DeviceSysopError::Unsupported);
        };
        return device_ns::ioctl_from_user_shared(
            device_handle,
