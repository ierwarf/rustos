...
pub struct BootFileHandle {
    inner: Arc<Mutex<BootFileState>>,
}

#[derive(Clone, Debug, Eq, PartialEq)]
pub struct BootDirectoryHandle {
    path: String,
}

impl BootDirectoryHandle {
...
impl BootFileHandle {
    pub fn read_only(path: String, bytes: Vec<u8>) -> Self {
        Self {
            inner: Arc::new(Mutex::new(BootFileState {
                path,
                bytes,
                cursor: 0,
                writable: false,
...
impl BootFileState {
    ...
    fn seek(&mut self, offset: i64, whence: BootFileSeekWhence) -> Result<u64, BootFileSeekError> {
...
pub enum KernelHandle {
    ...
    BootDirectory(BootDirectoryHandle),
...
mod tests {
    ...
    use super::{BootFileHandle, BootFileSeekError, BootFileSeekWhence, HandleTable, KernelHandle};

    #[test]
    fn read_at_preserves_stream_cursor() {
        let mut handle = BootFileHandle::read_only("/test".into(), vec![1, 2, 3, 4, 5]);
    ...
    fn seek_updates_cursor_with_linux_style_offsets() {
        let mut handle = BootFileHandle::read_only("/test".into(), vec![0; 8]);
    ...
    fn seek_rejects_positions_before_start_or_after_end() {
        let mut handle = BootFileHandle::read_only("/test".into(), vec![0; 4]);
    ...
    fn install_entry_min_keeps_existing_dynamic_fds() {
        ...
        let fd0 = table.install(KernelHandle::BootFile(BootFileHandle::read_only(
            "/a".into(),
            vec![1],
        )));
        let fd1 = table.install(KernelHandle::BootFile(BootFileHandle::read_only(
            "/b".into(),
            vec![2],
        )));
        let fd2 = table.install(KernelHandle::BootFile(BootFileHandle::read_only(
            "/c".into(),
            vec![3],
        )));
