...
// Serialize service/application image loads so the boot volume and dynamic loader do not
// contend through the service loop on debug/no-opt builds.
const MAX_LAUNCHES_PER_SERVICE: usize = 1;
const LAUNCH_COOLDOWN_MILLIS: u64 = 250;
const WAYLAND_DISPLAY_NAME: &str = "wayland-0";
...
macro_rules! trace_runtime {
    ($note:literal) => {
        crate::debug::record_trace_location_with_note(file!(), line!(), column!(), Some($note));
    };
}
...
impl DesktopRuntimeError {
    ...
    pub fn summary(&self) -> &'static str {
        match self {
            ...
            Self::Registry { .. } => "failed to load generated desktop registry",
        }
    }
    ...
    pub fn log_debug_details(&self) {
        match self {
            ...
            Self::Registry { path, error } => {
                debug::println!(
                    "desktop runtime registry error: path={} error={}",
                    path,
                    error
                );
            }
...
pub fn bootstrap() -> Result<(), DesktopRuntimeError> {
    with_runtime_state(|state| {
        ...
        state.programs.reserve(32);
        state.running.reserve(32);
...
pub fn snapshot_programs(dest: &mut [DesktopProgramInfo]) -> usize {
    trace_runtime!("runtime: snapshot programs");
    with_runtime_state(|state| {
...
pub fn snapshot_program_info(program_id: DesktopProgramId) -> Option<DesktopProgramInfo> {
    with_runtime_state(|state| {
        let program = state.programs.get(program_id.index())?;
        Some(DesktopProgramInfo {
            id: program_id,
            display_name: program.display_name,
            exec_path: program.exec_path,
            weight_micros: program.weight_micros,
...
pub fn snapshot_running_programs(dest: &mut [DesktopRunningProgramInfo]) -> usize {
    trace_runtime!("runtime: snapshot running programs");
    with_runtime_state(|state| {
...
pub fn request_launch(
    ...
    trace_runtime!("runtime: request launch");
    with_runtime_state(|state| {
        ...
        match target {
            ...
            DesktopLaunchTarget::NewSession => {
                ...
                debug::println!(
                    "desktop runtime launch requested: program_id={} name={} target=new-session",
                    program_id.index(),
...
fn process_requests() -> usize {
    ...
    let current_tick = crate::arch::rtc::ticks();
    let next_launch_tick = with_runtime_state(|state| state.next_launch_tick);
    if current_tick < next_launch_tick
        && with_runtime_state(|state| {
            matches!(state.requests.peek(), Some(DesktopRequest::Launch { .. }))
        })
    {
        return 0;
    }
    ...
    trace_runtime!("runtime: process requests pulled");
    for request in pulled {
        match request {
            DesktopRequest::Launch { .. } if launches_processed >= MAX_LAUNCHES_PER_SERVICE => {
                trace_runtime!("runtime: launch deferred max-per-service");
                deferred_launches.push(request);
            }
            DesktopRequest::Launch { .. } if current_tick < next_launch_tick => {
                trace_runtime!("runtime: launch deferred cooldown");
                deferred_launches.push(request);
            }
            DesktopRequest::Launch { program_id, target } => {
                trace_runtime!("runtime: launch dispatch");
                handle_launch_request(program_id, target);
                launches_processed += 1;
                processed += 1;
            }
            DesktopRequest::TerminateSession { session } => {
                trace_runtime!("runtime: terminate session dispatch");
                handle_terminate_session(session);
                processed += 1;
            }
            DesktopRequest::TerminatePid { pid } => {
                trace_runtime!("runtime: terminate pid dispatch");
    ...
    if !deferred_launches.is_empty() {
        trace_runtime!("runtime: requeue deferred requests");
        with_runtime_state(|state| {
...
fn handle_launch_request(program_id: DesktopProgramId, target: DesktopLaunchTarget) {
    trace_runtime!("runtime: handle launch begin");
    ...
    let target_label = match target {
        DesktopLaunchTarget::Session(_) => "session",
        DesktopLaunchTarget::NewSession => "new-session",
        DesktopLaunchTarget::AllSessions => "all-sessions",
    };
    debug::println!(
        "desktop runtime launch begin: program_id={} name={} exec={} target={} console_hosted={}",
        program_id.index(),
        program.display_name,
        program.exec_path,
        target_label,
        program.console_hosted,
    );

    let session_handle = if program.console_hosted {
        trace_runtime!("runtime: prepare console session");
    ...
    trace_runtime!("runtime: load executable image");
    ...
    let launch_env_storage =
        build_desktop_launch_env(image.bytes.as_ref(), program.env, program.logical_admin);
    trace_runtime!("runtime: build launch env");
    let launch_env = launch_env_storage
        .iter()
        .map(String::as_str)
        .collect::<Vec<_>>();
    let launch = console_host::ConsoleProgramSpec::new(
        image.bytes.as_ref(),
        program.exec_path,
        program.weight_micros,
    )
    .with_args(program.argv, launch_env.as_slice())
    .with_logical_admin(program.logical_admin);
    let console_session = session_handle.unwrap_or(ConsoleSessionHandle::SYSTEM);
    trace_runtime!("runtime: spawn program");
    match console_host::spawn_program_in_session(console_session, launch) {
        Ok(spawned) => {
            debug::println!(
                "desktop runtime launch spawned: program_id={} name={} pid={} session={}",
                program_id.index(),
                program.display_name,
                spawned.pid,
                console_session.raw()
            );
            let next_launch_tick =
                crate::arch::rtc::ticks().saturating_add(launch_cooldown_ticks());
            with_runtime_state(|state| {
                ...
                state.next_launch_tick = next_launch_tick;
            });
            if let Some(session_handle) = session_handle {
...
fn launch_cooldown_ticks() -> u64 {
    let ticks_per_second = crate::arch::rtc::ticks_per_second().max(1);
    let ticks = (LAUNCH_COOLDOWN_MILLIS
...
fn build_desktop_launch_env(image: &[u8], env: &[&str], logical_admin: bool) -> Vec<String> {
    let mut merged = env
        .iter()
    ...
    for entry in wayland_launch_env(logical_admin) {
        if let Some((key, _)) = entry.split_once('=') {
            if merged
                .iter()
                .any(|current| current.split_once('=').map(|(k, _)| k) == Some(key))
            {
                continue;
            }
        }
...
fn wayland_launch_env(logical_admin: bool) -> [String; 4] {
    let security = ProcessSecurityContext::new(logical_admin);
    let runtime_dir = alloc::format!("/run/user/{}", security.euid());
    [
        alloc::format!("XDG_RUNTIME_DIR={runtime_dir}"),
        alloc::format!("WAYLAND_DISPLAY={WAYLAND_DISPLAY_NAME}"),
...
fn prepare_console_session_for_launch(
    ...
    match target {
        ...
        DesktopLaunchTarget::NewSession | DesktopLaunchTarget::AllSessions => {
            trace_runtime!("runtime: create console session begin");
            let session =
            ...
            trace_runtime!("runtime: create console session done");
            trace_runtime!("runtime: transition console session loading begin");
            let _ = session::transition_console_session_state(
                session,
                ConsoleSessionState::LoadingImage,
            );
            trace_runtime!("runtime: transition console session loading done");
            Ok(session)
        }
    }
}
...
fn with_runtime_state<R>(f: impl FnOnce(&mut DesktopRuntimeState) -> R) -> R {
    // Runtime state is only touched from task context. Taking this lock with IRQs masked
    // can deadlock on a single CPU if the holder is preempted and another task spins on it.
    f(&mut RUNTIME.lock())
}
...
#[cfg(test)]
mod tests {
    use super::wayland_launch_env;

    #[test]
    fn wayland_launch_env_uses_desktop_runtime_dir_for_non_admin_processes() {
        let env = wayland_launch_env(false);
        assert_eq!(env[0], "XDG_RUNTIME_DIR=/run/user/1000");
        assert_eq!(env[1], "WAYLAND_DISPLAY=wayland-0");
    }

    #[test]
    fn wayland_launch_env_uses_root_runtime_dir_for_logical_admin_processes() {
        let env = wayland_launch_env(true);
        assert_eq!(env[0], "XDG_RUNTIME_DIR=/run/user/0");
    }
}
