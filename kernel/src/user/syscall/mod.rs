...
const SYSCALL_TRACE_BUDGET: usize = 0;
const SYSCALL_RETURN_TRACE_BUDGET: usize = 0;
...
pub(crate) fn set_linux_compat_current_task_ptr(current_task_ptr: usize) {
    unsafe {
        SYSCALL_CPU_LOCAL.linux_compat_current_task = current_task_ptr as u64;
    }
}

pub(crate) fn set_linux_compat_stack_guard(stack_guard: u64) {
    unsafe {
        SYSCALL_CPU_LOCAL.linux_compat_stack_guard = stack_guard;
    }
}

pub(crate) fn activate_linux_compat_cpu_local() {
    prepare_for_context_return(false, USER_GS_BASE_DEFAULT);
}

pub(crate) fn with_kernel_gs_base<T>(f: impl FnOnce() -> T) -> T {
...
pub(crate) const fn linux_compat_current_task_offset() -> usize {
    core::mem::offset_of!(SyscallCpuLocal, linux_compat_current_task)
}

pub(crate) const fn linux_compat_stack_guard_offset() -> usize {
...
fn trace_syscall_entry(frame: &SyscallFrame, abi: UserAbi) {
    let trace_index = SYSCALL_TRACE_COUNT.fetch_add(1, Ordering::Relaxed);
    if trace_index >= SYSCALL_TRACE_BUDGET {
        return;
    }

    let Some(snapshot) = multitask::current_user_snapshot() else {
        debug::println!(
            "syscall trace[{}]: abi={:?} pid=? tid=? nr={:#x} rip={:#x} rsp={:#x} rflags={:#x}",
            trace_index,
            abi,
            frame.rax,
            frame.user_rip,
...
fn trace_syscall_exit(frame: &SyscallFrame, abi: UserAbi, result: u64) {
    let trace_index = SYSCALL_RETURN_TRACE_COUNT.fetch_add(1, Ordering::Relaxed);
    if trace_index >= SYSCALL_RETURN_TRACE_BUDGET {
        return;
    }

    let Some(snapshot) = multitask::current_user_snapshot() else {
        debug::println!(
            "syscall return[{}]: abi={:?} nr={:#x} ret={:#x} rip={:#x} rsp={:#x} rflags={:#x}",
            trace_index,
            abi,
            frame.rax,
            result,
    ...
    debug::println!(
        "syscall return[{}]: abi={:?} pid={} tid={} nr={:#x} ret={:#x} rip={:#x} rsp={:#x} rflags={:#x}",
        trace_index,
        abi,
        snapshot.process_id(),
        snapshot.thread_id(),
