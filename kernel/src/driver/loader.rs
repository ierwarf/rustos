...
pub(super) fn load_module_image(
    ...
    mark_module_memory_executable(&memory)?;
...
pub(crate) fn runtime_executable_addr_is_known(addr: usize) -> bool {
...

    {
        ...
        if modules.iter().any(|module| {
            let start = module._memory.runtime_base();
            let end = start.saturating_add(module._memory.size());
...
impl KernelCompatTrampolinePage {
    fn new() -> Result<Self, &'static str> {
        ...
        if !crate::memory::paging::mark_direct_map_range_executable(
            crate::memory::paging::lower_half_addr(allocation.host_base() as u64),
            MODULE_PAGE_SIZE,
        ) {
            return Err("failed to mark compat trampoline page executable");
        }
        Ok(Self {
            allocation,
...
fn mark_module_memory_executable(memory: &ModuleMemory) -> Result<(), &'static str> {
    if crate::memory::paging::mark_direct_map_range_executable(
        ...
        Err("failed to mark module memory executable")
    }
}
...
#[cfg(test)]
mod tests {
    use super::{PendingModuleExecGuard, reset_for_tests, runtime_executable_addr_is_known};
