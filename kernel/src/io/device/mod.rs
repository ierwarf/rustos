...
pub(crate) fn read_to_current_user(
    handle: DeviceHandle,
    user_ptr: u64,
    user_len: usize,
) -> Result<usize, DeviceError> {
    match handle.device_id() {
        DeviceId::Input => input::read_to_current_user(user_ptr, user_len),
        DeviceId::Console | DeviceId::Display | DeviceId::Runtime => Err(DeviceError::Unsupported),
    }
}
...
pub(crate) fn ioctl_from_user_shared(
    handle: DeviceHandle,
    ...
    match handle.device_id() {
        DeviceId::Runtime => runtime::ioctl(process_state, request, arg),
        DeviceId::Console | DeviceId::Display | DeviceId::Input => Err(DeviceError::Unsupported),
    }
}
