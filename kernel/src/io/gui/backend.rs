...
pub(crate) fn install_boot_framebuffer(info: FramebufferInfo) -> bool {
    interrupts::without_interrupts(|| DISPLAY_BACKEND.lock().install_framebuffer(info))
}
...
pub(crate) fn present_bgra8888_from_user(
    ...
    let Some(result) = interrupts::without_interrupts(|| {
        DISPLAY_BACKEND.lock().with_framebuffer(|framebuffer| {
