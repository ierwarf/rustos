...
fn page_flags_from_win32_protect(protect: u32) -> Result<PageTableFlags, u32> {
    match protect {
        PAGE_READONLY => Ok(PageTableFlags::NO_EXECUTE),
        PAGE_READWRITE => Ok(PageTableFlags::WRITABLE | PageTableFlags::NO_EXECUTE),
        PAGE_EXECUTE_READ => Ok(PageTableFlags::empty()),
        PAGE_EXECUTE_READWRITE => Ok(PageTableFlags::WRITABLE),
        PAGE_NOACCESS => Err(ERROR_INVALID_PARAMETER),
