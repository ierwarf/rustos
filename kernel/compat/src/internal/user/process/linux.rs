...
pub(super) fn load_elf_file(file: VfsFileHandle) -> Result<LoadedProcessImage, ProcessLoadError> {
    ...
    let (entry, interpreter_base, max_loaded_end) =
        if let Some(interpreter_path) = interpreter_path.as_deref() {
            let interpreter_file = crate::vfs::open_path_for_kernel_file(interpreter_path)
...
fn elf_runtime_search_paths_file(
    ...
    let Some(dynamic_segment) = program_headers.iter().find(|ph| ph.kind == objelf::PT_DYNAMIC)
    else {
        return Ok(Vec::new());
    };
...
fn ensure_no_elf_interpreter_from_headers(
    ...
    if program_headers.iter().any(|ph| ph.kind == objelf::PT_INTERP) {
        return Err(ProcessLoadError::InvalidElf(
            "nested ELF interpreters are not supported",
        ));
    }
...
fn initialize_user_bytes_from_file(
    ...
    while current_file_offset < file_end {
        ...
        address_space.initialize_user_bytes(
            VirtAddr::new(current_user_addr),
            &chunk[..chunk_len],
        )?;
...
fn build_linux_process_image_from_headers(
    ...
    let program_headers_addr = program_header_table_addr_from_headers(program_headers, header, load_bias)?;
    let initial_tls = if interpreter_base == 0 {
...
fn elf_tls_template_info_from_headers(
    ...
    for ph in program_headers {
        ...
        let template_addr = ph.virtual_addr.checked_add(load_bias).ok_or(
            ProcessLoadError::InvalidElf("PT_TLS template address overflow"),
        )?;
        let template_end = template_addr.checked_add(template_size).ok_or(
            ProcessLoadError::InvalidElf("PT_TLS template bounds overflow"),
        )?;
        if template_end > paging::USER_SPACE_END_EXCLUSIVE {
            return Err(ProcessLoadError::InvalidElf(
                "PT_TLS template is outside the supported user range",
...
fn program_header_table_addr_from_headers(
    ...
    for ph in program_headers {
        if ph.kind == objelf::PT_PHDR {
            return ph.virtual_addr.checked_add(load_bias).ok_or(
                ProcessLoadError::InvalidElf("program header address overflow"),
            );
        }
    }
...
fn validate_segment_alignment_from_header(ph: &ParsedProgramHeader) -> Result<(), ProcessLoadError> {
    let align = ph.align;
    if align > 1 && !align.is_power_of_two() {
        return Err(ProcessLoadError::InvalidElf(
            "PT_LOAD alignment must be zero, one, or a power of two",
