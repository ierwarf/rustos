...
static MPROTECT_NOT_MAPPED_DIAG_BUDGET: AtomicU64 = AtomicU64::new(8);
...
pub(crate) fn munmap(start: u64, user_len: u64) -> Result<(), LinuxSysopError> {
    ...
    let Some(result) = multitask::with_current_user_process_state_mut(|_, abi, process_state| {
        ...
        let surface_segments = process_state.handles().surface_overlap_segments(start, end);
        let unmapped_len = if shared_segments.is_empty() && surface_segments.is_empty() {
            let (address_space, linux_process_state) =
                process_state.address_space_and_linux_process_state_mut();
            ...
            match address_space.protect_user_bytes(VirtAddr::new(start), len, page_flags) {
                ...
                Err(paging::AddressSpaceError::NotMapped) => {
                    ...
                    if !state.is_range_reserved(start, end) {
                        log_unexpected_mprotect_not_mapped(
                            address_space,
                            len,
                            exec_path.as_str(),
                            start,
...
fn log_unexpected_mprotect_not_mapped(
    ...
    crate::debug::println!(
        "linux mprotect unexpected NotMapped: exec={} start={:#x} end={:#x}",
        exec_path,
        start,
        end,
    );
    if let Some(area) = covering_vma {
        ...
        crate::debug::println!(
            "linux mprotect unexpected NotMapped: no covering VMA exec={} start={:#x} end={:#x}",
            exec_path,
            start,
            end,
        );
...
pub(crate) fn madvise(start: u64, user_len: u64, advice: u64) -> Result<(), LinuxSysopError> {
    if user_len == 0 {
        return Ok(());
    }
    if start & (PAGE_SIZE - 1) != 0 {
    ...
    let supported = matches!(
        advice,
        linux_abi::MADV_NORMAL
            | linux_abi::MADV_RANDOM
            | linux_abi::MADV_SEQUENTIAL
            | linux_abi::MADV_WILLNEED
            | linux_abi::MADV_DONTNEED
            | linux_abi::MADV_FREE
            | linux_abi::MADV_REMOVE
            | linux_abi::MADV_DONTFORK
            | linux_abi::MADV_DOFORK
            | linux_abi::MADV_MERGEABLE
            | linux_abi::MADV_UNMERGEABLE
            | linux_abi::MADV_HUGEPAGE
            | linux_abi::MADV_NOHUGEPAGE
            | linux_abi::MADV_DONTDUMP
            | linux_abi::MADV_DODUMP
            | linux_abi::MADV_WIPEONFORK
            | linux_abi::MADV_KEEPONFORK
            | linux_abi::MADV_COLD
            | linux_abi::MADV_PAGEOUT
            | linux_abi::MADV_POPULATE_READ
            | linux_abi::MADV_POPULATE_WRITE
            | linux_abi::MADV_DONTNEED_LOCKED
    );
    if !supported {
        return Err(LinuxSysopError::InvalidArgument);
    }
    ...
    let Some(result) = multitask::with_current_user_process_state(|_, abi, process_state| {
        ...
        if let Some(memory_map) = process_state.linux_memory_map() {
            let mapped = memory_map
                .areas()
                .iter()
