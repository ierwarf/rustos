...
pub(crate) fn read_path_for_current_process_to_vec(
    ...
    Ok(read_path_for_current_process_bytes(absolute_path)?.as_ref().to_vec())
}
...
fn take_opened_handle_for_current_process(absolute_path: &str) -> Result<KernelHandle, FileSysopError> {
