...
use kernel_object::api::{HandleOwner, HandleToken};
#[cfg(not(rustos_building_kernel_compat))]
use crate::object_tokens::{HandleOwner, HandleToken};
...
impl KernelHandle {
    ...
    pub(crate) const fn device_handle(&self) -> Option<DeviceHandle> {
        match self {
    ...
    pub(crate) fn procfs_link_target(&self, token: HandleToken) -> alloc::string::String {
        match self {
