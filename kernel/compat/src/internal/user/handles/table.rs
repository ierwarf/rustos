...
impl HandleEntry {
    pub(crate) fn new(handle: KernelHandle, fd_flags: u32, status_flags: u64) -> Self {
        Self {
            token: handle.token(),
            handle,
            fd_flags,
    ...
    pub(crate) fn handle_mut(&mut self) -> &mut KernelHandle {
        &mut self.handle
    }
    ...
    pub(crate) fn into_handle(self) -> KernelHandle {
        self.handle
    }

    pub(crate) fn fd_flags(&self) -> u32 {
        self.fd_flags
    }

    pub(crate) fn set_fd_flags(&mut self, fd_flags: u32) {
    ...
    pub(crate) fn set_status_flags(&mut self, status_flags: u64) {
...
impl HandleTable {
    ...
    pub(crate) fn install(&mut self, handle: KernelHandle) -> u64 {
        self.install_with_open_flags(handle, 0)
    }

    pub(crate) fn install_with_open_flags(&mut self, handle: KernelHandle, open_flags: u64) -> u64 {
    ...
    pub(crate) fn install_entry(&mut self, entry: HandleEntry) -> u64 {
        self.install_entry_min(entry, FIRST_DYNAMIC_FD as u64)
    }

    pub(crate) fn install_entry_min(&mut self, entry: HandleEntry, min_fd: u64) -> u64 {
    ...
    pub(crate) fn get(&self, fd: u64) -> Option<&KernelHandle> {
        Some(self.get_entry(fd)?.handle())
    }

    pub(crate) fn get_mut(&mut self, fd: u64) -> Option<&mut KernelHandle> {
        Some(self.get_entry_mut(fd)?.handle_mut())
    }

    pub(crate) fn get_entry(&self, fd: u64) -> Option<&HandleEntry> {
        let index = dynamic_index(fd)?;
        self.entries.get(index)?.as_ref()
    }

    pub(crate) fn get_entry_mut(&mut self, fd: u64) -> Option<&mut HandleEntry> {
        let index = dynamic_index(fd)?;
        self.entries.get_mut(index)?.as_mut()
    }

    pub(crate) fn ensure_entry_capacity(&mut self, index: usize) {
        if self.entries.len() <= index {
            self.entries.resize_with(index + 1, || None);
        }
    }

    pub(crate) fn replace_entry(&mut self, fd: u64, entry: Option<HandleEntry>) -> Option<()> {
        let index = dynamic_index(fd)?;
    ...
    pub(crate) fn close(&mut self, fd: u64) -> Option<KernelHandle> {
        let index = dynamic_index(fd)?;
        let handle = self
            .entries
            .get_mut(index)?
    ...
    pub(crate) fn clear_surface_mappings_in_range(&mut self, start: u64, len: u64) {
    ...
    pub(crate) fn surface_overlap_segments(&self, start: u64, end: u64) -> Vec<(u64, u64)> {
