...
impl SocketHandle {
    pub fn new_unix_stream_with_owner(owner: SocketCredentials) -> Self {
        Self {
    ...
    pub fn socketpair(credentials: SocketCredentials) -> (Self, Self) {
    ...
    pub fn bound_path(&self) -> Option<String> {
    ...
    pub fn owner_credentials(&self) -> SocketCredentials {
        self.inner.owner
    }

    pub fn peer_credentials(&self) -> Option<SocketCredentials> {
        let state = self.inner.state.lock();
        match &state.state {
    ...
    pub fn bind(&self, path: &str) -> Result<(), SocketError> {
    ...
    pub fn connect(&self, path: &str) -> Result<(), SocketError> {
    ...
    pub fn accept(&self, nonblocking: bool) -> Result<Self, SocketError> {
        loop {
            let maybe_pending = {
                let mut state = self.inner.state.lock();
    ...
    pub fn send(&self, src: &[u8], nonblocking: bool) -> Result<usize, SocketError> {
        if src.is_empty() {
            return Ok(0);
        }
    ...
    pub fn recv(&self, dest: &mut [u8], nonblocking: bool) -> Result<usize, SocketError> {
    ...
    pub fn poll_revents(&self, requested: i16) -> i16 {
        let mut ready = 0_i16;
        let maybe_peer = {
            let state = self.inner.state.lock();
            match &state.state {
    ...
    pub fn shutdown(&self, how: u64) -> Result<(), SocketError> {
