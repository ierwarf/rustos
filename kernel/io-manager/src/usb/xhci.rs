...
fn handle_async_event(controller: &mut XhciControllerState, event: XhciEvent) {
    match event {
        XhciEvent::Transfer {
            ...
            {
                let Some(device) = controller.devices[device_index].as_mut() else {
                    crate::debug::println!(
                        "xhci transfer event raced with device removal: slot={} ep_id={}",
                        slot_id,
                        ep_id
                    );
                    return;
            ...
            if should_resubmit {
                ...
                let Some(device) = controller.devices[device_index].as_mut() else {
                    crate::debug::println!(
                        "xhci poll resubmit skipped after device removal: slot={} ep_id={}",
                        slot_id,
                        ep_id
                    );
                    return;
