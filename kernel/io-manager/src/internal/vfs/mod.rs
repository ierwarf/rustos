...
impl RootCache {
    ...
    fn insert_file(&mut self, generation: u64, path: &str, bytes: Arc<[u8]>) {
        ...
        if let Some(index) = self.files.iter().position(|entry| entry.path == path) {
            self.total_file_bytes = self.total_file_bytes.saturating_sub(self.files[index].bytes.len());
            ...
            || self.total_file_bytes.saturating_add(bytes.len()) > ROOT_FILE_CACHE_MAX_BYTES
        {
            ...
            self.total_file_bytes =
                self.total_file_bytes.saturating_sub(self.files[index].bytes.len());
            self.files.remove(index);
        }
...
pub(crate) fn open_path_for_kernel_file(absolute_path: &str) -> Result<VfsFileHandle, VfsError> {
    let absolute_path = normalize_kernel_path(absolute_path)?;
    match open_local_path(
...
impl VfsFileObject for RootFsStreamingFile {
    ...
    fn write_at(&self, _offset: usize, _src: &[u8]) -> Result<usize, crate::user::handles::FileHandleWriteError> {
...
pub(crate) fn read_dir_names_for_kernel(absolute_path: &str) -> Result<Vec<String>, VfsError> {
    let absolute_path = normalize_kernel_path(absolute_path)?;
    Ok(
        read_root_dir_cached(absolute_path.as_str())?
            .into_iter()
...
fn open_local_path(absolute_path: &str, flags: u64, _mode: u64) -> Result<VfsOpenResult, VfsError> {
    ...
    match metadata.kind {
        ...
        VfsNodeKind::File => {
            ...
            let file_len =
                usize::try_from(metadata.len).map_err(|_| VfsError::InvalidArgument)?;
