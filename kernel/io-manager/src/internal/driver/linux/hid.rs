...
struct CompatObject<const N: usize>(UnsafeCell<[u8; N]>);

unsafe impl<const N: usize> Sync for CompatObject<N> {}

impl<const N: usize> CompatObject<N> {
...
static HID_BUS_TYPE: CompatObject<128> = CompatObject::zeroed();
static HID_OPS: CompatObject<64> = CompatObject::zeroed();
...
pub(crate) fn bus_type_ptr() -> *const c_void {
    HID_BUS_TYPE.as_mut_ptr().cast_const()
}
...
pub(crate) unsafe extern "C" fn __hid_register_driver(
    ...
    if status != 0 {
        crate::debug::write_debugcon_only_line(
            alloc::format!(
                "hid register driver: driver_register failed ptr={:#x} status={}",
...
pub(crate) unsafe extern "C" fn hid_hw_start(
    ...
    if ll.is_null() {
        crate::debug::write_debugcon_only_line(
            alloc::format!("hid_hw_start: dev={:#x} ll=null", dev as usize).as_bytes(),
        );
        return 0;
    }
    let Some(start) = (unsafe { (*ll).start }) else {
        crate::debug::write_debugcon_only_line(
            alloc::format!(
                "hid_hw_start: dev={:#x} ll={:#x} start=null",
                dev as usize, ll as usize
            )
            .as_bytes(),
        );
        return 0;
    ...
    crate::debug::write_debugcon_only_line(
        alloc::format!(
            "hid_hw_start: begin dev={:#x} ll={:#x} start={:#x}",
            dev as usize, ll as usize, start as usize
        )
        .as_bytes(),
    );
    ...
    crate::debug::write_debugcon_only_line(
        alloc::format!("hid_hw_start: end dev={:#x} status={}", dev as usize, status).as_bytes(),
    );
    status
}
...
pub(crate) unsafe extern "C" fn hid_hw_open(dev: *mut LinuxCompatHidDevice) -> i32 {
    ...
    if ll.is_null() {
        crate::debug::write_debugcon_only_line(
            alloc::format!("hid_hw_open: dev={:#x} ll=null", dev as usize).as_bytes(),
        );
        return 0;
    }
    unsafe {
    ...
    let Some(open) = (unsafe { (*ll).open }) else {
        crate::debug::write_debugcon_only_line(
            alloc::format!(
                "hid_hw_open: dev={:#x} ll={:#x} open=null",
                dev as usize, ll as usize
            )
            .as_bytes(),
        );
    ...
    crate::debug::write_debugcon_only_line(
        alloc::format!(
            "hid_hw_open: begin dev={:#x} ll={:#x} open={:#x} open_count={}",
            dev as usize,
            ll as usize,
            open as usize,
            unsafe { (*dev).ll_open_count }
        )
        .as_bytes(),
    );
    ...
    crate::debug::write_debugcon_only_line(
        alloc::format!(
            "hid_hw_open: end dev={:#x} status={} open_count={}",
            dev as usize,
            status,
            unsafe { (*dev).ll_open_count }
        )
...
pub(crate) fn resolve_symbol(name: &str) -> Option<usize> {
    match name {
        ...
        "hid_ops" => Some(HID_OPS.as_mut_ptr() as usize),
        _ => None,
    }
}
...
fn bind_device_to_driver(dev: *mut LinuxCompatHidDevice, driver: *mut LinuxCompatHidDriver) -> i32 {
    ...
    crate::debug::write_debugcon_only_line(
        alloc::format!(
            "hid bind begin: driver={} dev={:#x} vendor={:04x} product={:04x} bus={:04x}",
            driver_name,
            dev as usize,
            unsafe { (*dev).vendor },
            unsafe { (*dev).product },
    ...
    let status = if let Some(probe) = unsafe { (*driver).probe } {
        crate::debug::write_debugcon_only_line(
            alloc::format!(
                "hid bind probe begin: driver={} dev={:#x} probe={:#x}",
                driver_name, dev as usize, probe as usize
            )
            .as_bytes(),
        );
        unsafe { probe(dev, id) }
    ...
    crate::debug::write_debugcon_only_line(
        alloc::format!(
            ...
            driver_name, dev as usize, status
        )
        .as_bytes(),
    );
    if status != 0 {
        ...
        crate::debug::write_debugcon_only_line(
            alloc::format!(
                "hid bind end: driver={} dev={:#x} status={}",
                driver_name, dev as usize, status
            )
            .as_bytes(),
        );
        ...
        crate::debug::write_debugcon_only_line(
            alloc::format!(
                "hid bind auto-open begin: driver={} dev={:#x}",
                driver_name, dev as usize
            )
            .as_bytes(),
        );
        ...
        if open_status != 0 {
            crate::debug::write_debugcon_only_line(
                alloc::format!(
                    "hid bind auto-open failed: driver={} dev={:#x} status={}",
                    driver_name, dev as usize, open_status
                )
                .as_bytes(),
            );
        }
        crate::debug::write_debugcon_only_line(
            alloc::format!(
                "hid bind end: driver={} dev={:#x} status={} open_status={}",
                driver_name, dev as usize, status, open_status
            )
            .as_bytes(),
        );
    }
