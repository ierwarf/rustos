...
pub(crate) fn on_key_event_for_session(session: ConsoleSessionHandle, event: KeyboardEvent) {
...
pub(crate) fn read_input_for_session(session: ConsoleSessionHandle, dest: &mut [u8]) -> usize {
    interrupts::without_interrupts(|| TTY.lock().session_mut(session).read_input(dest))
}

pub(crate) fn has_pending_input_for_session(session: ConsoleSessionHandle) -> bool {
    interrupts::without_interrupts(|| TTY.lock().session_mut(session).has_pending_input())
}

pub(crate) fn pending_input_len_for_session(session: ConsoleSessionHandle) -> usize {
    interrupts::without_interrupts(|| TTY.lock().session_mut(session).pending_input_len())
}

pub(crate) fn termios_for_session(session: ConsoleSessionHandle) -> linux_abi::LinuxTermios {
    interrupts::without_interrupts(|| TTY.lock().session_mut(session).termios())
}

pub(crate) fn set_termios_for_session(
...
pub(crate) fn read_input_blocking_for_session(
    session: ConsoleSessionHandle,
    dest: &mut [u8],
) -> usize {
    if dest.is_empty() {
